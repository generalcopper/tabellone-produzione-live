...
const docDetailPhotosMeta = document.getElementById("docDetailPhotosMeta");
    ...
    const btnCloseDocDetail2 = document.getElementById("btnCloseDocDetail2");
    ...
    (function __liftOverlaysToBody(){
      try{
        ["viewOcr","viewInventory","viewFlows","viewAnag"].forEach(id => {
    ...
    function setView(name){
      ...
      const overlayKeys = ["ocr","inventory","flows","anag"];
    ...
    function startHomeOcr(){
      ...
      requestAnimationFrame(() => {
        if (__isMobileDevice()) cameraInput.click();
        else galleryInput.click();
      });
    }

    document.getElementById("btnGoOcr")?.addEventListener("click", startHomeOcr);
    document.getElementById("btnGoInv")?.addEventListener("click", openInventoryOverlay);
...
document.getElementById("btnCloseAnag")?.addEventListener("click", (e) => { try{ e.preventDefault(); e.stopPropagation(); }catch(_){} setView("home"); });
...
// UI selection
    ...
    function openInventoryOverlay(){
      setView("inventory");
      setInventoryWarehouse(""); // mostra picker
...
/****************************************************************
    ...
    let state = {
      ...
      productCategories: {} // per-code macro category (offline fallback)
    };
    ...
    function startRealtime() {
      ...
      fb.unsub.suppliers = onSnapshot(
        ...
        (err) => {
          ...
          setSyncPill("bad", "Sync: permessi fornitori");
        }
      );
      ...
      fb.unsub.products = onSnapshot(
        ...
        (err) => {
          ...
          setSyncPill("bad", "Sync: permessi prodotti");
        }
      );
      ...
      fb.unsub.movements = onSnapshot(
        ...
        (err) => {
          ...
          setSyncPill("bad", "Sync: permessi movimenti");
        }
      );
    }
  ...
  try{
    ...
    flowEditItemsTbody.innerHTML = rows.map(r => `
      <tr data-mvid="${escapeHtmlAttr(r.id)}">
        ...
        <td data-label="Codice"><span class="kbd">${escapeHtml(r.code || "—")}</span></td>
...
async function deleteMovementsBulk(ids) {
  ...
  saveLocalData();
    ...
    async function saveFlowEdit() {
      ...
      const customer = String(flowEditCustomer ? flowEditCustomer.value : "").trim();
      const date = String(flowEditDate ? flowEditDate.value : "").trim();
      const note = String(flowEditNote ? flowEditNote.value : "").trim();

      if (!customer) return openModal("Dato mancante", "Inserisci il fornitore.");
      if (!date) return openModal("Dato mancante", "Inserisci la data.");

      const ids = (g.movements || []).map(m => m.id).filter(Boolean);
      try {
        btnSaveFlowEdit && (btnSaveFlowEdit.disabled = true);
        await updateMovementsBulk(ids, { customer, date, note });
        showToast("Flusso aggiornato");
        closeFlowEdit();
      } catch (e) {
        console.error("saveFlowEdit failed", e);
        openModal("Errore", "Non sono riuscito a salvare le modifiche.");
        btnSaveFlowEdit && (btnSaveFlowEdit.disabled = false);
      } finally {
        btnSaveFlowEdit && (btnSaveFlowEdit.disabled = false);
      }
    }
...
async function deleteMovement(id) {
  ...
  return mv;
    ...
    function renderStats(stockArr) {
      ...
      const low = stockArr.filter(x => (Number(x.qty) || 0) < (Number(x.threshold) || 0)).length;
    ...
    function renderLowStockBoard(stockByWh) {
      try {
        ...
        const isLow = (r) => (Number(r && r.qty) || 0) < (Number(r && r.threshold) || 0);
...
function renderDocDetailPhotos(g){
  ...
  pages.forEach(async (p, idx) => {
    ...
    const MACRO_CATEGORIES = ["scatole", "divisori", "flaconi", "trigger", "tappi", "bobine", "materie_prime", "non_classificati"];

    function normalizeMacroCategory(v) {
      const s = String(v || "").trim().toLowerCase();
      return MACRO_CATEGORIES.includes(s) ? s : "";
    }
...
let __stockRowByKey = new Map();
    ...
    function renderStockTable(stockArr) {
      ...
      stockTbody.innerHTML = show.map(r => {
        ...
        const qtyCell = isGroup
          ? `<div class="qty-editor" style="justify-content:flex-end;"><span style="font-weight:900; color:${qtyColor};">${Number(qtyVal).toLocaleString("it-IT")}</span></div>`
          : `
        ...
        const catHtml = cat ? `<span class="pill" style="padding:2px 8px;">${escapeHtml(macroCatLabel(cat))}</span>` : '<span class="td-muted">—</span>';
    ...
    function renderFlowsTable() {
      ...
      if (docs.length === 0) {
        flowsTbody.innerHTML = '<tr><td class="td-muted" colspan="5">Nessun flusso ancora.</td></tr>';
        return;
      }
          ...
          <tr class="docRow" data-dockey="${escapeHtmlAttr(g.key)}" title="Apri dettaglio">
            ...
            <td data-label="Azioni" class="td-actions">
              <div class="inlineRow flowActRow" style="justify-content:flex-end;">
                <button class="btn btn-ghost btn-xs" type="button" data-action="editFlow" title="Modifica flusso">Modifica</button>
                <button class="btn btn-danger btn-xs" type="button" data-action="deleteFlow" title="Elimina flusso">Elimina</button>
              </div>
            </td>
          </tr>
        `;
...
function renderAll() {
      ...
      if (__currentWarehouse) {
        ...
        const invRows = (stockByWh || []).filter(x => normalizeWarehouse(x.warehouse) === wh);
    ...
    function renderAnag() {
      ...
      if (activeAnagTab === "products") {
        ...
        if (!fb.user) {
          anagTbody.innerHTML = `<tr><td class="td-muted" colspan="4">Accedi con Google per sincronizzare i prodotti.</td></tr>`;
          return;
        }

        const list = Array.isArray(products) ? products : [];

        // raggruppa per alias (se presente), altrimenti singolo per codice
        const map = new Map();
        for (const p of list) {
          if (!p) continue;
          ...
          const aliasKey = alias ? ("alias:" + normTextKey(alias)) : "";
          const key = aliasKey || ("code:" + code.toLowerCase());

          const label = (alias || String(p.name || code)).trim();
          const cur = map.get(key) || { key, label, alias, items: [] };
          ...
          cur.items.push({ code, name: String(p.name || code).trim(), alias: alias });
          map.set(key, cur);
        }

        const groups = Array.from(map.values()).map(g => {
          const codes = g.items.map(x => x.code).filter(Boolean);
          const codesUniq = Array.from(new Set(codes));
          g.codes = codesUniq;
          g.count = codesUniq.length;
          return g;
        }).sort((a,b) => String(a.label||"").localeCompare(String(b.label||""), "it", { sensitivity:"base" }));

        // filtro ricerca (furbo: ignora spazi/maiuscole)
        ...
        __prodGroupsMap = new Map(filtered.map(g => [g.key, g]));
        ...
        anagTbody.innerHTML = filtered.slice(0, 600).map(g => {
          const codes = (g.codes || []);
          const preview = codes.slice(0, 2).join(", ");
          const more = Math.max(0, codes.length - 2);
          const codesTxt = preview + (more ? ` +${more}` : "");
          const badge = (codes.length > 1) ? `<span class="pill" style="margin-left:8px;">Unificati</span>` : ``;
          ...
          <tr>
            ...
            <td data-label="Azioni" class="td-actions">
              <div class="inlineRow" style="justify-content:flex-end; gap:8px;">
                ...
                <button class="btn btn-danger btn-xs" type="button" data-action="deleteSupplier" data-id="${escapeHtmlAttr(s.id)}">Elimina</button>
              </div>
            </td>
          </tr>
        `;
...
async function deleteSupplierCascade(supplierId){
  ...
  try{
    ...
    function openProductModal(id, ctx) {
      ...
      const code = String((ctx && ctx.code) || id || "").trim();
      if (!code) return;
      ...
      const headerHtml = `
        ...
        <div class="field" style="grid-column: 1 / -1;">
          ...
          <div class="td-muted" style="margin-top:6px;">Se due articoli hanno lo stesso alias: vengono <strong>unificati</strong> (es. tappo d50 + tappo ferrari d50).</div>
        </div>
      `;
  ...
  `);
      ...
      baseFields.push(`
        <div class="field" style="grid-column: 1 / -1;">
          ...
          <div class="td-muted" style="margin-top:6px;">
            Clicca per associare il prodotto a: scatole, divisori, flaconi, trigger, tappi, bobine, materie prime, non classificati.
          </div>
        </div>
      `);
      ...
      if (!isAliasGroup && !mode && isStockCtx) {
        ...
        stockHtml = `
          ...
          <div class="field" style="grid-column: 1 / -1;">
            ...
            <div class="td-muted" style="margin-top:6px;">Sposta una quantità (anche parziale) in un altro inventario (crea 2 movimenti: OUT + IN).</div>
          </div>
        `;
      }
      ...
      if (!mode && isStockCtx && !isAliasGroup) {
        ...
        invDangerHtml = `
          <div class="field" style="grid-column: 1 / -1;">
            ...
            <div class="td-muted" style="margin-top:6px;">
              Elimina tutte le righe/movimenti per questo <strong>Cliente + Codice</strong> (anche su entrambe le sedi). Operazione irreversibile.
...
// Danger zone: elimina prodotto (solo da Anagrafica Prodotti)
      ...
      if (mode === "master" && !isAliasGroup) {
        dangerHtml = `
          <div class="field" style="grid-column: 1 / -1;">
            ...
            <div class="td-muted" style="margin-top:6px;">Elimina solo l’anagrafica prodotto (nome/alias/categoria). Non elimina movimenti o stock.</div>
          </div>
        `;
      }
      ...
      prodFields.querySelectorAll("button[data-open-code]").forEach(btn => {
        btn.addEventListener("click", (e) => {
          ...
          openProductModal(c, mem || { code: c, __mode: "" });
        });
      });
      ...
      if (tInput && tSave && !isAliasGroup) {
        ...
        try {
          const hasOverride = (thresholds && // Warehouse move (single only)
      ...
      if (delInvBtn && !mode && isStockCtx && !isAliasGroup) {
        delInvBtn.addEventListener("click", async (e) => {
          ...
          const ok = confirm(
            ...
            `Nota: verranno eliminate tutte le righe/movimenti per questo Cliente+Codice (anche su entrambe le sedi).\n` +
          ...
          try {
            ...
            if (ids.length) await deleteMovementsBulk(ids);
...
async function handleFileSelection(fileList) {
      ...
      try {
        ...
        for (let i = startIdx; i < capture.files.length; i++) {
          const pageNum = i + 1;
          progressLabel.textContent = `OCR pagina ${pageNum}/${total}…`;

          // convert if HEIC
          const converted = await maybeConvertHeic(capture.files[i]);
          capture.files[i] = converted;

          // refresh preview if active page was converted
          if (capture.activePageIndex === i) {
            capture.file = converted;
            const url2 = URL.createObjectURL(converted);
            previewImg.src = url2;
            previewImg.onload = () => URL.revokeObjectURL(url2);
          }

          const data = await runGeminiOCR(converted);
          ...
          const pct = Math.min(95, Math.round(10 + 85 * (done / Math.max(1, total))));
    ...
    function exportStockCSV() {
      ...
      const rows = [
        ["Cliente","Codice","Articolo","Pezzi","Soglia","UltimoMovimento"]
    ...
    function exportMovementsCSV() {
      const rows = [
        ["Id","Data","Tipo","Cliente","Codice","Articolo","Pezzi","Note","Fonte","CreatoIl"]
...
/****************************************************************
    ...
    function saveManualModalMovement() {
      ...
      if (!validateMovementFields(mv)) {
        openModal("Dati mancanti", "Compila almeno: Cliente, Codice, Articolo, Pezzi (>0), Data.");
        return;
      }
    ...
    if (segSuppliers && segProducts) {
      segSuppliers.addEventListener("click", () => { activeAnagTab = "suppliers"; segSuppliers.classList.add("active"); segProducts.classList.remove("active"); renderAnag(); });
      segProducts.addEventListener("click", () => { activeAnagTab = "products"; segProducts.classList.add("active"); segSuppliers.classList.remove("active"); renderAnag(); });
    }
...
// Product modal
    ...
    if (anagTbody) {
      anagTbody.addEventListener("click", (e) => {
        // Click riga (prodotti): apri gruppo
        if (activeAnagTab === "products") {
          ...
          if (tr && (!e.target.closest || !e.target.closest("button[data-action]"))) {
            ...
            if (gid) { openProductAliasGroup(gid); return; }
          }
        }
        ...
        if (action === "openProdGroup") { openProductAliasGroup(id); return; }
      });
    }
btnConfirmMovement.addEventListener("click", async () => {
      ...
      if (!customer) {
        openModal("Cliente mancante", "Non riesco a leggere il nome fornitore dal documento. Riprova con una foto più nitida.");
        return;
      }
      if (!items.length) {
...
// Upsert prodotti (best effort)
    ...
    if (btnCloseDocDetail2) btnCloseDocDetail2.addEventListener("click", closeDocDetail);
...
if (modalFlowEdit) {
  ...
  // Commit qty on blur (focusout bubbles)
  modalFlowEdit.addEventListener("focusout", async (e) => {
    const inp = e.target && e.target.closest ? e.target.closest("input.jsFlowQtyInput") : null;
    if (!inp) return;

    const id = String(inp.dataset.id || "").trim();
    if (!id) return;

    let v = safeInt(inp.value);
    if (!Number.isFinite(v) || v < 0) v = 0;

    const orig = safeInt(inp.dataset.orig);
    if (v === orig) {
    ...
    inp.disabled = true;
    try {
      await updateMovementsBulk([id], { qty: v });
      inp.dataset.orig = String(v);
      showToast("Quantità aggiornata");
      try { renderFlowEditItems(); } catch(_){}
    } catch (err) {
      console.error("update flow qty failed", err);
      openModal("Errore", "Non sono riuscito a salvare la quantità.");
      inp.value = String(orig);
    } finally {
      inp.disabled = false;
    }
    ...
    if (flowsTbody) flowsTbody.addEventListener("click", async (e) => {
      const tr = e.target.closest('tr[data-dockey]');
      if (!tr) return;
      ...
      if (actionBtn) {
        ...
        if (act === "editFlow") return openFlowEdit(key);
        if (act === "deleteFlow") return deleteFlowByKey(key);
        return;
      }
