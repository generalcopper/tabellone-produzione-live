// ilovepaghe.bundle.js — merged (dashboard.js + index.js)
// Generated: 2026-01-31
// Note: include this file as a <script type="module"> (same as original index.js).
globalThis.ILOVEPAGHE_BUNDLE_BUILD = "20260211_001_pages_basefix";

// dashboard.js — iLovePaghe (SPA dashboard)
// Build: 20260131_013_dirfix
globalThis.ILOVEPAGHE_DASHBOARD_BUILD = "20260211_001_pages_basefix";
const initPagheiaDashboard = () => {
  const root = document.getElementById("dashboardView");
  // Se lo script viene caricato prima del DOM (es. <head> senza defer),
  // aspetta il DOMContentLoaded e riprova: altrimenti la dashboard resta vuota.
  if(!root){
    try{
      if(document.readyState === "loading"){
        if(!globalThis.__PAGHEIA_DASH_WAIT_DOM__){
          globalThis.__PAGHEIA_DASH_WAIT_DOM__ = true;
          document.addEventListener("DOMContentLoaded", ()=>{
            try{ initPagheiaDashboard(); }catch(_e){}
          }, { once:true });
        }
      }
    }catch(_e){}
  ...
  // ===== Config Firebase (stesso progetto di ilovepaghe.js) =====
  const firebaseConfig = {
    apiKey: "AIzaSyD_2Eb6ni7E08hUbEkozP85LzyfesutO6M",
    authDomain: "www.ilovepaghe.com",
  ...
  const DEFAULT_BILLING_ENDPOINT = "/"; // Same-origin (Firebase Hosting rewrite -> Cloud Run, no CORS)
  const COL_PAYROLL_USAGE = "payrollUsage";
  const COL_USER_PREFS = "userPrefs";
  const LS_SAVE_EMAILS = "payroll_save_emails_v1"; // compatibilità con ilovepaghe.js

  // Premium (Stripe) — marker locale per sincronizzare il piano al rientro dal checkout
  const LS_ILP_PREMIUM_CHECKOUT_PENDING = "ilp_premium_checkout_pending_v1";
  const SS_ILP_PREMIUM_CHECKOUT_STARTED = "ilp_premium_checkout_started_v1";
  function __ilpJsonParse(v){
    try{ return JSON.parse(String(v||"")); }catch(_e){ return null; }
  }
  function ilpMarkPremiumCheckoutPending(sessionId){
    try{
      const sid = String(sessionId || "").trim();
      localStorage.setItem(LS_ILP_PREMIUM_CHECKOUT_PENDING, JSON.stringify({ ts: Date.now(), session_id: sid }));
    }catch(_e){}
    try{ sessionStorage.setItem(SS_ILP_PREMIUM_CHECKOUT_STARTED, String(Date.now())); }catch(_e){}
  }
  function ilpReadPremiumCheckoutPending(maxAgeMs = 2 * 60 * 60 * 1000){
    try{
      const raw = localStorage.getItem(LS_ILP_PREMIUM_CHECKOUT_PENDING);
      if(!raw) return null;
      const d = __ilpJsonParse(raw);
      if(!d || !d.ts) return null;
      const age = Date.now() - Number(d.ts || 0);
      if(!(age >= 0) || age > maxAgeMs){
        try{ localStorage.removeItem(LS_ILP_PREMIUM_CHECKOUT_PENDING); }catch(_e){}
        return null;
      }
      return d;
    }catch(_e){ return null; }
  }
  function ilpClearPremiumCheckoutPending(){
    try{ localStorage.removeItem(LS_ILP_PREMIUM_CHECKOUT_PENDING); }catch(_e){}
    try{ sessionStorage.removeItem(SS_ILP_PREMIUM_CHECKOUT_STARTED); }catch(_e){}
  }
  function ilpCheckoutStartedRecently(maxAgeMs = 2 * 60 * 60 * 1000){
    try{
      const ts = parseInt(sessionStorage.getItem(SS_ILP_PREMIUM_CHECKOUT_STARTED) || "0", 10);
      if(!Number.isFinite(ts) || ts <= 0) return false;
      return (Date.now() - ts) < maxAgeMs;
    }catch(_e){ return false; }
  }
  function ilpCameFromStripe(){
    try{
      const ref = String(document.referrer || "").toLowerCase();
      return ref.includes("stripe.com");
    }catch(_e){ return false; }
  }
  ...
  // ===== UI helpers =====
  function toast(_msg, _ms=2200){
    // NO-OP: toast UI disabled by request.
  }
  ...
  function setLoading(on){
    ...
    try{ el.setAttribute("aria-hidden", on ? "false" : "true"); }catch(_e){}
  }
  ...
  function setInert(el, on){
    ...
    try{
      ...
      try{ if(on) el.setAttribute("inert", ""); else el.removeAttribute("inert"); }catch(_e2){}
    }
  }
  ...
  function openSidebar(){
    ...
    // opzionale: porta il focus dentro al menu su mobile
    try{
      if(window.matchMedia && window.matchMedia("(max-width: 920px)").matches){
        side?.querySelector?.(".navItem")?.focus?.();
      }
    }catch(_e){}
  }
  function closeSidebar(){
    ...
    if(isMobile){
      // Evita aria-hidden su un elemento che contiene il focus
      try{
        const active = document.activeElement;
        if(side && active && side.contains(active)){
          const hamb = $("btnHamb");
          if(hamb) hamb.focus();
        }
      }catch(_e){}
  ...
  async function initFirebase(){
    ...
    let appCheck = null;
    try{
      // Evita doppia init (ilovepaghe.js può averlo già inizializzato)
      if(globalThis.__PAGHEIA_APPCHECK__){
        appCheck = globalThis.__PAGHEIA_APPCHECK__;
      }else if(APPCHECK_ENABLED && appCheckInit?.initializeAppCheck && appCheckInit?.ReCaptchaV3Provider && APPCHECK_SITE_KEY){
        appCheck = appCheckInit.initializeAppCheck(app, {
          provider: new appCheckInit.ReCaptchaV3Provider(APPCHECK_SITE_KEY),
          isTokenAutoRefreshEnabled: true
        });
        globalThis.__PAGHEIA_APPCHECK__ = appCheck;
      }
    }catch(_e){}
const auth = getAuth(app);
    ...
    return {
      app, auth, db,
      api: { doc, getDoc, setDoc, onSnapshot },
  ...
  async function readPlanFromFirestore(env, uid, email){
    try{
      const { api, db } = env;
      ...
      const ids = [];
      if(uidStr) ids.push(uidStr);
      if(emailLower && emailLower !== uidStr) ids.push(emailLower);
      ...
      for(const id of ids){
        try{
          const ref = api.doc(db, COL_PAYROLL_USAGE, id);
          ...
          const d = snap.data() || {};
          // Nota: eventuale migrazione legacy (emailLower → uid) gestita altrove; qui ascoltiamo SOLO uid.

          const plan = String(d.plan || d.tier || d.subscription || "").toLowerCase();
          const status = String(d.status || d.subscriptionStatus || d.stripeSubscriptionStatus || d.stripeStatus || "").toLowerCase();
          const isPremium =
            (d.isPremium === true) ||
            (d.premium === true) ||
            (plan === "premium" || plan === "pro" || plan === "plus") ||
            (status === "active" || status === "trialing");

          return { ok:true, exists:true, isPremium, plan, raw:d, idUsed:id };
        }catch(err){
          lastErr = err;
        }
      }

      if(!anyRead && lastErr) return { ok:false, err:lastErr };
  ...
  function applyPlanUI(planState){
    ...
    if(btnGoPremium){
      btnGoPremium.style.display = (!isPremium && !pending) ? "" : "none";
      btnGoPremium.disabled = !!pending;
    }
    if(btnManage){
      btnManage.style.display = isPremium ? "" : "none";
      btnManage.disabled = !!pending;
    }

    // Top CTA (header): cambia in base al piano
    const topCta = $("btnGoPremiumTop");
    if(topCta){
      const mode = pending ? "pending" : (isPremium ? "manage" : "upgrade");
      topCta.dataset.mode = mode;
      topCta.disabled = !!pending;
      topCta.classList.toggle("manage", !!isPremium && !pending);
      topCta.classList.toggle("pending", !!pending);
      topCta.textContent = pending ? "Verifica…" : (isPremium ? "Gestisci abbonamento" : "Diventa Premium");
    }

    // Sidebar status label (testo + colore)
    const sideStatus = $("sideUserStatus");
    if(sideStatus){
      sideStatus.textContent = pending ? "Verifica" : (isPremium ? "Premium" : "Free");
      sideStatus.classList.remove("premium","free","pending");
      sideStatus.classList.add(pending ? "pending" : (isPremium ? "premium" : "free"));
    }
  }
  ...
  try{
    ...
    // ✅ Checkout OK: torna alla HOME (come richiesto)
    const u = new URL("", base);
    u.searchParams.set("premium","success");
    u.searchParams.set("session_id","{CHECKOUT_SESSION_ID}");
return u.toString();
  }catch(_e){
    return location.origin + "/?premium=success&session_id={CHECKOUT_SESSION_ID}";
  }
})();
const cancelUrl = (() => {
  try{
    const base = new URL("./", import.meta.url);
    // ✅ Checkout annullato (tasto “indietro” su Stripe): torna a "Diventa Premium"
    const u = new URL("premium.html", base);
    u.searchParams.set("premium","cancel");
    return u.toString();
  }catch(_e){
    return location.origin + "/premium.html?premium=cancel";
  }
})();
    ...
    const resp = await fetch(endpoint, {
      ...
      body: JSON.stringify({ successUrl, cancelUrl, returnUrl: location.href })
    });

    const data = await resp.json().catch(()=> ({}));
    if(!resp.ok || !data?.url) throw new Error(data?.error || "Impossibile avviare il checkout.");
    const isPortal = !!(data?.alreadyPremium || data?.hasSubscription);
    if(!isPortal){
      try{ ilpMarkPremiumCheckoutPending(""); }catch(_e){}
    }
    ...
    async function openCustomerPortal(env, user){
      ...
      // UX: resta su questa pagina mentre il backend prepara l'URL del portale.
      // Evita il tab "about:blank" che può restare visibile per alcuni secondi.
      if(globalThis.__ILP_PORTAL_OPENING__) return;
      globalThis.__ILP_PORTAL_OPENING__ = true;
      ...
      try{
        try{
          if(loadingEl) loadingEl.textContent = "Apro gestione abbonamento…";
          setLoading(true);
        }catch(_e){}

        // Lascia un frame al browser per mostrare l'overlay prima delle chiamate di rete
        try{ await new Promise(r=>requestAnimationFrame(()=>r())); }catch(_e){}

        const tok = await user.getIdToken(true);
        const returnUrl = (() => {
    try{
      const base = new URL("./", import.meta.url);
      // ✅ Ritorno robusto: torna alla HOME e apre la dashboard (evita problemi su /dashboard in full-load)
      const u = new URL("", base);
      u.searchParams.set("open","dashboard");
      u.hash = "#plans";
      return u.toString();
    }catch(_e){
      return location.origin + "/?open=dashboard#plans";
    }
  })();
        ...
        // Apri nel tab corrente: evita popup/tab vuoti e funziona ovunque.
        navigating = true;
        location.href = data.url;
      }finally{
        try{ globalThis.__ILP_PORTAL_OPENING__ = false; }catch(_e){}
        if(!navigating){
          try{ if(loadingEl) loadingEl.textContent = prevText || "Caricamento in corso…"; }catch(_e){}
          try{ setLoading(false); }catch(_e){}
        }
      }
    }
  ...
  // ===== UI: mini spinner / verifica salvataggio email su Firebase =====
  const __SAVEEMAILS_SYNC__ = { pending:0, timer:null };

  function __ensureSaveEmailsSyncStyle(){
    try{
      if(document.getElementById("ilp-saveemails-sync-style")) return;
      const style = document.createElement("style");
      style.id = "ilp-saveemails-sync-style";
      style.textContent = `
        .ilp-saveemails-sync{display:inline-flex;align-items:center;justify-content:center;width:16px;height:16px;margin-left:8px;vertical-align:middle;opacity:.9}
        .ilp-saveemails-sync .ilp-dot{width:10px;height:10px;border-radius:999px;border:2px solid currentColor;opacity:.25;box-sizing:border-box}
        .ilp-saveemails-sync .ilp-spin{width:10px;height:10px;border-radius:999px;border:2px solid currentColor;border-top-color:transparent;animation:ilp-mini-spin .75s linear infinite;box-sizing:border-box}
        .ilp-saveemails-sync .ilp-ok,.ilp-saveemails-sync .ilp-err{font-size:12px;line-height:1}
        @keyframes ilp-mini-spin{to{transform:rotate(360deg)}}
      `;
      (document.head || document.documentElement).appendChild(style);
    }catch(_e){}
  }

  function ensureSaveEmailsSyncIndicator(){
    try{
      let el = document.getElementById("saveEmailsSyncIndicator");
      if(el) return el;

      const toggle = document.getElementById("toggleSaveEmails");
      if(!toggle) return null;

      __ensureSaveEmailsSyncStyle();

      el = document.createElement("span");
      el.id = "saveEmailsSyncIndicator";
      el.className = "ilp-saveemails-sync";
      el.setAttribute("role","status");
      el.setAttribute("aria-live","polite");
      el.title = "Pronto";
      el.innerHTML = '<span class="ilp-dot"></span>';

      // Evita di rompere la struttura dello switch: inserisci DOPO il <label> (se presente)
      const switchLabel = toggle.closest("label");
      if(switchLabel){
        switchLabel.insertAdjacentElement("afterend", el);
      }else{
  ...
  function setSaveEmailsSyncState(state, detail){
    const el = ensureSaveEmailsSyncIndicator();
    if(!el) return;
    ...
    if(s === "saving"){
      el.innerHTML = '<span class="ilp-spin"></span>';
      el.title = detail || "Salvataggio su Firebase in corso…";
      return;
    }
    if(s === "ok"){
      el.innerHTML = '<span class="ilp-ok">✓</span>';
      el.title = detail || "Salvato";
      return;
    }
    if(s === "err"){
      el.innerHTML = '<span class="ilp-err">⚠</span>';
      el.title = detail || "Errore salvataggio";
      return;
    }

    el.innerHTML = '<span class="ilp-dot"></span>';
    el.title = detail || "Pronto";
  }

  function beginSaveEmailsSync(detail){
    try{
      __SAVEEMAILS_SYNC__.pending = (__SAVEEMAILS_SYNC__.pending || 0) + 1;
      if(__SAVEEMAILS_SYNC__.timer){
        clearTimeout(__SAVEEMAILS_SYNC__.timer);
        __SAVEEMAILS_SYNC__.timer = null;
      }
      setSaveEmailsSyncState("saving", detail || "Salvataggio…");
    }catch(_e){}
  }

  function endSaveEmailsSync(ok, detail){
    try{
      __SAVEEMAILS_SYNC__.pending = Math.max(0, (__SAVEEMAILS_SYNC__.pending || 0) - 1);

      if((__SAVEEMAILS_SYNC__.pending || 0) > 0){
        setSaveEmailsSyncState("saving", "Salvataggio…");
        return;
      }

      setSaveEmailsSyncState(ok ? "ok" : "err", detail || (ok ? "Salvato" : "Non verificato su Firebase"));
      __SAVEEMAILS_SYNC__.timer = setTimeout(()=>{
        try{
          if((__SAVEEMAILS_SYNC__.pending || 0) === 0) setSaveEmailsSyncState("idle", "Pronto");
        }catch(_e){}
      }, ok ? 1400 : 2600);
    }catch(_e){}
  }
  ...
  // Stato piano locale (UI) + watcher per sincronizzare tra tab/browser.
  // Source of truth resta Firestore (scritto dal webhook Stripe) o GET /entitlements.
  let LAST_PLAN_STATE = { isPremium:false, pending:false };
  let PLAN_WATCH_UNSUB = null;

  function stopPlanWatch(){
    try{ if(typeof PLAN_WATCH_UNSUB === "function") PLAN_WATCH_UNSUB(); }catch(_e){}
    PLAN_WATCH_UNSUB = null;
  }

  function startPlanWatch(user){
    stopPlanWatch();
    try{
      if(!ENV?.ok || !user || user.isAnonymous) return;
      const api = ENV.api;
      if(!api || typeof api.onSnapshot !== "function") return;
      const uid = user.uid;
      if(!uid) return;

      const ref = api.doc(ENV.db, COL_PAYROLL_USAGE, uid);
      PLAN_WATCH_UNSUB = api.onSnapshot(ref, snap=>{
        try{
          ...
          // Mantieni "pending" solo se il marker è ancora valido (TTL corto), altrimenti sblocca UI.
          let pending = !!LAST_PLAN_STATE.pending;
          try{
            if(!ilpReadPremiumCheckoutPending(2 * 60 * 1000)) pending = false;
          }catch(_e){ pending = false; }

          LAST_PLAN_STATE = { isPremium, pending: (isPremium ? false : pending) };

          if(isPremium){
            try{ ilpClearPremiumCheckoutPending(); }catch(_e){}
            // Stop polling eventuale
            try{
              const st = globalThis.__ILP_DASH_PREMIUM_POLL__;
              if(st?.timer){ clearTimeout(st.timer); st.timer = null; }
              if(st) st.n = 0;
            }catch(_e){}
          }

          applyPlanUI(LAST_PLAN_STATE);
        }catch(_e){}
      }, _err=>{
        // Ignore: refreshPlan() gestisce fallback /entitlements
      });
    }catch(_e){}
  }
  ...
  async function refreshPlan(forceToken=false){
    ...
    // Rientro Stripe / verifica piano (robusta)
    const url = new URL(location.href);
    const p = url.searchParams;
    const premiumFlag = String(p.get("premium") || "").toLowerCase();
    const sid = p.get("session_id") || p.get("sid") || p.get("checkout_session_id") || "";
    const succ = String(p.get("success") || "").toLowerCase();
    const isSuccess = (premiumFlag === "success") || (succ === "1" || succ === "true" || succ === "ok" || succ === "yes") || !!sid;
    const isCancel = (premiumFlag === "cancel") || (p.get("premium_cancel") === "1") || (String(p.get("cancel")||"") === "1") || (String(p.get("canceled")||"") === "1");
    const fromStripe = ilpCameFromStripe();
    const lsPending = ilpReadPremiumCheckoutPending(2 * 60 * 60 * 1000);
    const startedRecent = ilpCheckoutStartedRecently(2 * 60 * 60 * 1000);

    // Se annullato, pulisci marker (evita "verifica" fantasma)
    if(isCancel){
      try{ ilpClearPremiumCheckoutPending(); }catch(_e){}
    }

    // Se abbiamo segnali di rientro checkout, segna pending (anche se mancano i parametri standard)
    let shouldSync = (isSuccess || fromStripe || !!lsPending || startedRecent) && !isCancel;

    // ✅ Se l'utente ha aperto il checkout ma NON ha completato (nessun session_id, nessun ritorno da Stripe),
    // non restare bloccato su "Verifica…": torna subito a "Diventa Premium".
    // (Caso tipico: chiude Stripe o torna indietro senza completare)
    try{
      const hasSidInMarker = !!(lsPending && String(lsPending.session_id || "").trim());
      const looksAbandoned = !!lsPending && !hasSidInMarker && !isSuccess && !fromStripe && !sid;
      if(looksAbandoned){
        try{ ilpClearPremiumCheckoutPending(); }catch(_e){}
        shouldSync = false;
      }
    ...
    if(shouldSync){
      // Evita di "refreshare" continuamente il marker durante i poll: altrimenti il timestamp si resetta sempre.
      try{
        const shouldRefreshMarker = (!lsPending) || isSuccess || fromStripe || startedRecent || !!sid;
        if(shouldRefreshMarker){
          ilpMarkPremiumCheckoutPending(sid || (lsPending?.session_id || ""));
        }
      }catch(_e){}
    }

    // Pulisci URL (evita ripetizioni al refresh)
    try{
      const had = (p.has("premium") || p.has("success") || p.has("session_id") || p.has("sid") || p.has("checkout_session_id") || p.has("premium_cancel") || p.has("cancel") || p.has("canceled"));
      if(had){
        ["premium","success","session_id","sid","checkout_session_id","premium_cancel","cancel","canceled"].forEach(k=>{ try{ p.delete(k); }catch(_e){} });
        history.replaceState({}, "", url.toString());
      }
    }catch(_e){}

    const pending = !!shouldSync;

    let planState = { isPremium:false, pending: !!pending };

    // Source of truth: Firestore (payrollUsage/{uid}) o GET /entitlements.
    // 1) Firestore payrollUsage/{uid}
    const fs = await readPlanFromFirestore(ENV, user.uid, user.email);
    if(fs.ok && fs.exists){
      ...
      // 2) Fallback backend (/entitlements) — evita UI bloccata se Firestore non è leggibile o è in ritardo
      planState.pending = !!pending;
      try{
        const base = getBillingBase();
        if(base){
          const endpoint = base.replace(/\/$/,"") + "/entitlements";
          const tok = await user.getIdToken(true);
          const resp = await fetch(endpoint, {
            method:"GET",
            headers:{
          ...
          const data = await resp.json().catch(()=> ({}));
          if(resp.ok && data){
            const plan = String(data.plan || data.tier || data.subscription || "").toLowerCase().trim();
            const status = String(data.subscriptionStatus || data.status || data.stripeSubscriptionStatus || data.stripeStatus || "").toLowerCase().trim();
            const prem = (data.isPremium === true) || (data.premium === true) || (plan === "premium" || plan === "pro" || plan === "plus") || (status === "active" || status === "trialing");
            if(prem){
    ...
    try{ LAST_PLAN_STATE = { ...planState }; }catch(_e){}

    // Auto-sync dopo checkout: riprova per un breve periodo e poi sblocca lo stato
    try{
      globalThis.__ILP_DASH_PREMIUM_POLL__ = globalThis.__ILP_DASH_PREMIUM_POLL__ || { n:0, timer:null };
      const st = globalThis.__ILP_DASH_PREMIUM_POLL__;

      if(st.timer){ try{ clearTimeout(st.timer); }catch(_e){} st.timer = null; }

      if(planState.isPremium){
        st.n = 0;
        try{ ilpClearPremiumCheckoutPending(); }catch(_e){}
      }else if(planState.pending){
        st.n = Math.min(60, (Number(st.n||0) + 1));
        if(st.n < 60){
          st.timer = setTimeout(()=>{ try{ refreshPlan(true); }catch(_e){} }, 2500);
        }else{
          // Timeout: mai lasciare la UI bloccata su “Verifica…”
          st.n = 0;
          try{ ilpClearPremiumCheckoutPending(); }catch(_e){}
          try{
            planState.pending = false;
          ...
          try{ toast("Pagamento ricevuto: sto ancora verificando. Se tra poco non cambia, premi “Verifica” o ricarica."); }catch(_e){}
        }
      }else{
        st.n = 0;
      }

      globalThis.__ILP_DASH_PREMIUM_POLL__ = st;
    }catch(_e){}
  }
  ...
  async function bindActions(){
    $("btnGoApp")?.addEventListener("click", ()=>{
      if(typeof globalThis.ilovepagheSpaNavigate === "function"){
        globalThis.ilovepagheSpaNavigate("home");
        return;
      }
    ...
    // Top CTA: se Premium -> apre portale, altrimenti va a "Diventa Premium"
    $("btnGoPremiumTop")?.addEventListener("click", async ()=>{
      try{
        const btn = $("btnGoPremiumTop");
        ...
        if(mode === "pending"){
          toast("Verifica in corso…");
          return;
        }
        // upgrade (default)
        location.href = "/premium.html";
      }catch(err){
        toast(err?.message || "Operazione non disponibile.");
      }
    });

    $("btnManagePlan")?.addEventListener("click", async ()=>{
      const btn = $("btnManagePlan");
      try{
        if(btn){
  ...
  async function renderSettings(user){
    ...
    // Mini indicatore (spinner/check) accanto a "Vuoi salvare mail"
    try{ ensureSaveEmailsSyncIndicator(); setSaveEmailsSyncState("idle", "Pronto"); }catch(_e){}
    ...
    ENV.authMod.onAuthStateChanged(ENV.auth, async (user)=>{
      try{
        ...
        if(!user || user.isAnonymous){
          ...
          { const _el = $("sideUserPlan"); if(_el) _el.textContent = "—"; }
        ...
        try{ startPlanWatch(user); }catch(_e){}
...
// Boot robusto (funziona anche se lo script è in <head> senza defer)
try{
  if(document.readyState === "loading"){
    document.addEventListener("DOMContentLoaded", ()=>{
      try{ initPagheiaDashboard(); }catch(_e){}
...
// Build: 20260130_008
    globalThis.ILOVEPAGHE_BUILD = "20260212_002_savepdf_switch";
...
const firebaseConfig = {
    ...
    const APPCHECK_SITE_KEY = "6LcwcVUsAAAAAK4A6obrpGbGFHYGw3Wparj1626K"; // reCAPTCHA v3 site key (public)
    // App Check su GitHub Pages/localhost può fallire (dominio non autorizzato o script bloccati).
    // Per evitare che l'app si rompa in test/dev, lo disabilitiamo automaticamente su github.io/localhost.
    // Override:
    //   globalThis.PAGHEIA_FORCE_APPCHECK = true;   // forza ON
    //   globalThis.PAGHEIA_DISABLE_APPCHECK = true; // forza OFF
    const APPCHECK_ENABLED = (() => {
          try{
            const host = String(location.hostname || "").toLowerCase();
            const forced = String(globalThis.PAGHEIA_FORCE_APPCHECK || "").toLowerCase() === "true";
            ...
            const hardAllow = new Set(["ilovepaghe.com","www.ilovepaghe.com"]);
            if(hardAllow.has(host)) return true;
            ...
            // Default: OFF su domini non previsti (evita errori reCAPTCHA inutili)
            return false;
          }catch(_e){
    ...
    // PDF: salvataggio su Firebase Storage + metadati su Firestore (accesso riservato).
    // Nota: l’invio email resta via endpoint (Cloud Run mailer), ma i PDF vengono archiviati per consultazione.
    const PAYROLL_PERSIST_UPLOADS = true; // true = salva PDF su Storage/DB (privacy: accesso solo account autorizzati)
...
// Billing (Stripe) — Premium 19,90 €/mese
    ...
    const PAYROLL_PREMIUM_PRICE_LABEL = "19,90 €/mese";
    const PAGHEIA_BILLING_ENDPOINT = "/"; // Same-origin (Firebase Hosting rewrite -> Cloud Run billing, no CORS)
    ...
    // =========================
    // Conversioni (GA4 / Google Ads) — Premium (Stripe)
    // - La Home è la pagina di ritorno del checkout: tracciamo la conversione SOLO quando Premium risulta attivo.
    // - Evita falsi positivi (es. refresh della Home).
    // - Event name di default: "ads_conversion_PURCHASE_1" (puoi cambiarlo da GA4 o via window.ILP_GA4_PREMIUM_EVENT_NAME)
    // =========================
    const ILP_GA4_PREMIUM_EVENT_NAME = String(globalThis.ILP_GA4_PREMIUM_EVENT_NAME || "ads_conversion_PURCHASE_1");
    const ILP_PREMIUM_VALUE_EUR = (() => {
      try{
        const raw = globalThis.ILP_PREMIUM_VALUE_EUR ?? globalThis.PAYROLL_PREMIUM_VALUE_EUR ?? "";
        const n = Number(String(raw).replace(",", "."));
        return (Number.isFinite(n) && n > 0) ? n : 19.90;
      }catch(_e){
        return 19.90;
      }
    })();
    const ILP_PREMIUM_CURRENCY = String(globalThis.ILP_PREMIUM_CURRENCY || "EUR");

    const LS_ILP_PREMIUM_CHECKOUT_PENDING = "ilp_premium_checkout_pending_v1";
    const LS_ILP_PREMIUM_CHECKOUT_TRACKED = "ilp_premium_checkout_tracked_v1";
    const SS_ILP_PREMIUM_CHECKOUT_STARTED = "ilp_premium_checkout_started_v1";
    ...
    function ilpMarkPremiumCheckoutPending(sessionId){
      try{
        const sid = String(sessionId || "").trim();
        const payload = { ts: Date.now(), session_id: sid };
        localStorage.setItem(LS_ILP_PREMIUM_CHECKOUT_PENDING, JSON.stringify(payload));
      }catch(_e){}
      try{ sessionStorage.setItem(SS_ILP_PREMIUM_CHECKOUT_STARTED, String(Date.now())); }catch(_e){}
    }

    function ilpReadPremiumCheckoutPending(maxAgeMs = 6 * 60 * 60 * 1000){
      try{
        const raw = localStorage.getItem(LS_ILP_PREMIUM_CHECKOUT_PENDING);
        if(!raw) return null;
        const d = __ilpJsonParse(raw);
    ...
    function ilpHasTrackedPremiumCheckout(sessionId){
      try{
        const raw = localStorage.getItem(LS_ILP_PREMIUM_CHECKOUT_TRACKED);
        if(!raw) return false;
        const d = __ilpJsonParse(raw);
        if(!d || !d.ts) return false;
        // se manca session id, proteggiamo da duplicati ravvicinati (1h)
        if(!sessionId) return (Date.now() - Number(d.ts||0)) < (60 * 60 * 1000);
        return String(d.session_id || "") === String(sessionId || "");
      }catch(_e){
        return false;
      }
    }

    function ilpMarkPremiumCheckoutTracked(sessionId){
      try{
        const sid = String(sessionId || "").trim();
        localStorage.setItem(LS_ILP_PREMIUM_CHECKOUT_TRACKED, JSON.stringify({ ts: Date.now(), session_id: sid }));
      }catch(_e){}
      try{ localStorage.removeItem(LS_ILP_PREMIUM_CHECKOUT_PENDING); }catch(_e){}
    }

    function ilpTryTrackPremiumConversion(isPremium){
      try{
        if(!isPremium) return;
        const pending = ilpReadPremiumCheckoutPending();
        if(!pending) return;

        const sid = String(pending.session_id || "").trim();
        if(ilpHasTrackedPremiumCheckout(sid)){
          try{ localStorage.removeItem(LS_ILP_PREMIUM_CHECKOUT_PENDING); }catch(_e){}
          return;
        }

        const tx = sid || ("ilp_premium_" + String(pending.ts || Date.now()));
        const params = { value: ILP_PREMIUM_VALUE_EUR, currency: ILP_PREMIUM_CURRENCY, transaction_id: tx };

        // 1) GA4 event (da usare come "conversione" in GA4 -> Key events)
        try{
          if(typeof gtag === "function"){
            gtag("event", ILP_GA4_PREMIUM_EVENT_NAME, params);
          }
        }catch(_e){}

        // 2) (Opzionale) Google Ads direct conversion tag (se vuoi usare AW-.../LABEL)
        //    Imposta in index.html: window.ILP_GOOGLE_ADS_SEND_TO = "AW-XXXXXXX/XXXXXXXXXXXX";
        try{
          const sendTo = String(globalThis.ILP_GOOGLE_ADS_SEND_TO || globalThis.ILP_ADS_SEND_TO || "").trim();
          if(sendTo && typeof gtag === "function"){
            gtag("event", "conversion", { send_to: sendTo, ...params });
          }
        }catch(_e){}

        ilpMarkPremiumCheckoutTracked(sid);
      }catch(_e){}
    }
    const state = {
      user: null,
    ...
    // =========================
    const LS_AUTH_NEXT_URL = "ilovepaghe_auth_next_url_v1";
    const LS_EMAIL_LINK_EMAIL = "ilovepaghe_email_link_email_v1";
    const LS_EMAIL_LINK_SENT_AT = "ilovepaghe_email_link_sent_at_v1";
...
// =========================
    ...
    const SS_PAGHEIA_AUTH_INFLIGHT = "ilovepaghe_auth_inflight_ts_v1";
    const SS_PAGHEIA_AUTH_INTENT = "ilovepaghe_auth_inflight_intent_v1";
    const SS_PAGHEIA_AUTH_REASON = "ilovepaghe_auth_inflight_reason_v1";
...
const toast = document.getElementById("toast");
    ...
    function showToast(_title, _body="", _ms=2600){
      // NO-OP: toast UI disabled by request.
    }
...
if(payrollGreeting) payrollGreeting.textContent = `Ciao, ${name}.`;
    ...
    function updateAuthUI(){
      ...
      // Safety net: non lasciare mai “Verifica…” infinito (es. errori rete / regole Firestore / webhook in ritardo)
      try{
        if(state.user?.premiumSyncPending){
          const t0 = Number(state.user.premiumSyncStartedAt || 0);
          const age = t0 ? (Date.now() - t0) : 0;
          if(t0 && age >= 120000){
            state.user.premiumSyncPending = false;
            try{ ilpClearPremiumCheckoutPending(); }catch(_e){}
          }
        }
      }catch(_e){}
  ...
  if(pill){
    ...
    pill.setAttribute("aria-label", isAuthed ? (isPremium ? "Account Premium" : "Account") : "Account");
      ...
      try{
        ...
        // Menu: pulsante Home (non deve mai mostrare "Carica")
        const navUp = document.getElementById("btnNavUpload");
        if(navUp){
          navUp.textContent = "Home";
          try{ navUp.setAttribute("aria-label","Home"); }catch(_e){}
        }
      }catch(_e){}
...
updateGreetingUI();
      ...
      // Conversione Premium: scatta solo quando Premium è effettivamente attivo
      try{ ilpTryTrackPremiumConversion(!!isPremium); }catch(_e){}
// Cross-page header sync (marketing pages): snapshot auth state to avoid "flash" on other pages
try{
  const __KEY = "ilovepaghe_auth_snapshot_v1";
  if(state && state.authHydrated){
    if(isAuthed){
      const payload = {
        v: 1,
        ts: Date.now(),
        ...
        name: String(state.user?.displayName || state.user?.email || "Utente"),
        isPremium: !!isPremium
      };
      if(payload.email) localStorage.setItem(__KEY, JSON.stringify(payload));
    }else{
    ...
    function updateProfileDropdownCaret(){
      try{
        const dd = document.getElementById("profileDropdown");
        const card = dd?.querySelector(".profileDropdownCard");
        const pill = document.getElementById("pillAuth");
        if(!dd || !card || !pill) return;
        const cardRect = card.getBoundingClientRect();
        const paddingRight = 22;
        const caretX = Math.max(0, cardRect.width - paddingRight);
        card.style.setProperty("--profile-caret-x", `${caretX}px`);
      }catch(_e){}
    }
    ...
    function toggleProfileDropdown(show){
      ...
      if(on){
        try{ requestAnimationFrame(updateProfileDropdownCaret); }catch(_e){ updateProfileDropdownCaret(); }
      }
    }
...
// Pagamento/Upgrade Premium (opzionale):
    ...
    async function startPremiumCheckout(nextUrl){
      // Anti doppio-click / multi-tab: evita doppie Checkout Session
      if(globalThis.__ILP_CHECKOUT_OPENING__) return;
      globalThis.__ILP_CHECKOUT_OPENING__ = true;
      ...
      try{
        try{ await state.firebase?.ensureAppCheck?.(); }catch(_e){}
  ...
  try{
    ...
    return location.origin + "/premium.html?premium=cancel";
      ...
      const res = await fetch(endpoint, {
        method: "POST",
        headers,
        body: JSON.stringify({ successUrl, cancelUrl, returnUrl: (nextUrl || location.href) })
      });
      ...
      const isPortal = !!(data?.alreadyPremium || data?.hasSubscription);
      if(isPortal){
        try{ showToast("Sei già Premium", "Apro gestione abbonamento…", 2200); }catch(_e){}
      }else{
        // Marker locale: al rientro da Stripe forziamo refresh token/entitlements anche se mancano query params
        try{ ilpMarkPremiumCheckoutPending(""); }catch(_e){}
      }
      location.href = url;
      return;
    }finally{
      try{ globalThis.__ILP_CHECKOUT_OPENING__ = false; }catch(_e){}
    }
    }
    ...
    function goToPremium(reason=""){
      ...
      try{
        ...
        const securityOpen = document.getElementById("securityOverlay")?.getAttribute("aria-hidden") === "false";
        const anyOpen = !!(adminOpen || logOpen || pdfOpen || premiumOpen || authOpen || pendingOpen || securityOpen);
...
function toggleSendLogModal(show){
    ...
    const Payroll = (()=>{
      ...
      const Ve = (title, body)=>{
        const safeTitle = String(title || "Info");
        const safeBody = String(body || "");
        const normalizedBody = safeBody.trim().toLowerCase();
        // Evita toast inutili quando il selettore file viene chiuso senza scegliere un file.
        if(normalizedBody === "nessun file selezionato") return;
        try{ showToast(safeTitle, safeBody); }catch(_e){}
      };
      ...
      // payrollDirectory docId: evita collisioni tra clienti (multi-tenant)
      function payrollDirDocId(emailLower){
        try{
          const em = String(emailLower || "").trim().toLowerCase();
          const u = (N && N.user && N.user.uid) ? String(N.user.uid) : "";
          if(!u) return em;
          if(!em) return "";
          return u + "__" + em;
        }catch(_e){
          try{ return String(emailLower || "").trim().toLowerCase(); }catch(_e2){ return ""; }
        }
      }
...
const LS_PAYROLL_FREE_USED = "ilovepaghe_payroll_free_used_v1";
      ...
      // Preferenza: Salva i PDF su Firebase (switch nel menu laterale Match)
      const LS_PAYROLL_SAVE_PDFS = "payroll_save_pdfs_v1";
      const PAYROLL_SAVE_PDFS_DEFAULT = true; // default ON

      function payrollReadSavePdfsPref(){
        try{
          const v = localStorage.getItem(LS_PAYROLL_SAVE_PDFS);
          if(v === null || v === undefined) return PAYROLL_SAVE_PDFS_DEFAULT;
          return String(v) !== "0";
        }catch(_e){
          return PAYROLL_SAVE_PDFS_DEFAULT;
        }
      }
      function payrollWriteSavePdfsPref(on){
        try{ localStorage.setItem(LS_PAYROLL_SAVE_PDFS, on ? "1" : "0"); }catch(_e){}
      }
      function payrollShouldPersistUploads(){
        try{
          const a = N.payroll?.admin;
          if(a && typeof a.savePdfs === "boolean") return a.savePdfs;
        }catch(_e){}
        return payrollReadSavePdfsPref();
      }
      function payrollUserCanPersistPdfs(){
        try{
          const u = N.user;
          return !!(u && !u.isAnonymous && u.emailLower);
        }catch(_e){
          return false;
        }
      }

      // Lazy load Firebase Storage (solo se serve)
      let __ilpEnsureStoragePromise = null;
      async function ensureFirebaseStorageReady(){
        try{
          if(N.firebase?.storage && N.firebase?.storageApi) return true;
          if(__ilpEnsureStoragePromise) return await __ilpEnsureStoragePromise;

          __ilpEnsureStoragePromise = (async ()=>{
            const app = N.firebase?.app;
            if(!app) return false;

            const FIREBASE_VER = "12.7.0";
            const bases = [];
            // prova prima la base già usata dal loader (se presente)
            try{
              const b = String(N.firebase?.__firebaseLoadedBase || "").trim();
              if(b) bases.push(b);
            }catch(_e){}
            bases.push(
              "https://www.gstatic.com/firebasejs/" + FIREBASE_VER + "/",
              "https://cdn.jsdelivr.net/npm/firebase@" + FIREBASE_VER + "/",
              "https://unpkg.com/firebase@" + FIREBASE_VER + "/"
            );
            ...
            for(const base of bases){
              try{
                storageMod = await import(base + "firebase-storage.js");
                if(storageMod && storageMod.getStorage) break;
              }catch(err){ lastErr = err; }
            }
            if(!storageMod || !storageMod.getStorage){
              console.warn("firebase-storage load failed", lastErr);
              return false;
            }

            try{
              const storage = storageMod.getStorage(app);
              N.firebase.storage = storage;
              N.firebase.storageApi = {
                ref: storageMod.ref,
          ...
          return await __ilpEnsureStoragePromise;
        }catch(err){
          console.warn("ensureFirebaseStorageReady", err);
          return false;
        }
      }
      ...
      function readLocalPayrollEntitlements(){
        try{
          ...
          // Premium: niente override locale (source of truth = Firestore/entitlements)
        }catch(_e){}
      }
      ...
      async function tryLoadPayrollEntitlementsFromBillingBackend(){
        // Fallback: chiedi lo status al backend billing (GET /entitlements).
        // Utile se:
        // - le regole Firestore bloccano la lettura client-side, oppure
        // - vogliamo forzare uno stato "fresh" subito dopo Stripe.
        try{
          const base = getBillingBase();
          ...
          // Token utente
          let tok = "";
          try{ tok = await authUser.getIdToken(true); }catch(_e){}
          if(!tok) return false;

          const headers = { "Authorization": "Bearer " + tok, "Cache-Control": "no-store" };

          // App Check token (best effort)
          try{
            try{ await N.firebase?.ensureAppCheck?.(); }catch(_e){}
          ...
          const plan = String(data.plan || data.tier || data.subscription || "").toLowerCase().trim();
          const status = String(data.subscriptionStatus || data.status || data.stripeSubscriptionStatus || data.stripeStatus || "").toLowerCase().trim();
          const isPrem = (data.isPremium === true) || (data.premium === true) || (plan === "premium" || plan === "pro" || plan === "plus") || (status === "active" || status === "trialing");
      ...
      async function loadPayrollEntitlementsFromServer(force=false){
        ...
        const ids = [];
        // ✅ Compatibilità: prima uid, poi (fallback) emailLower.
        // Se il backend ha scritto lo status Premium su docId=emailLower, lo intercettiamo e lo migriamo su uid.
        if(uid) ids.push(uid);
        if(emailLower && emailLower !== uid) ids.push(emailLower);

        // Premium: source of truth = Firestore (/payrollUsage) o GET /entitlements. Niente Custom Claims.
        ...
        try{
          ...
          if(!snap || !snap.exists()){
            ...
            // Se la sincronizzazione Premium dura troppo, sblocca lo stato "Verifica…"
            try{
              const pending = !!N.user?.premiumSyncPending;
              const t0 = Number(N.user?.premiumSyncStartedAt || 0);
              const age = t0 ? (Date.now() - t0) : 0;
          ...
          const plan = String(d.plan || d.tier || d.subscription || "").toLowerCase();
          const status = String(d.status || d.subscriptionStatus || d.stripeSubscriptionStatus || d.stripeStatus || "").toLowerCase();
          const isPrem = (d.isPremium === true) || (d.premium === true) || (plan === "premium" || plan === "pro" || plan === "plus") || (status === "active" || status === "trialing");
          let freeUsed = false;
      ...
      function togglePromptOverlay(overlayId, show){
        ...
        const focusKey = "__lastFocusPremiumPrompt__";
      ...
      async function runPostSendPrompts({ sentAny=false, sendOk=false, trialJustUsed=false } = {}){
        // Se la prova si è appena conclusa: mostra Premium
      ...
      try{
        document.getElementById("btnNavUpload")?.addEventListener("click", ()=>{
          // Niente route "carica": il bottone deve semplicemente tornare alla HOME
          try{ toggleAdminModal(false); }catch(_e){}
          try{
            try{ navigateSpaToRoute("home"); }catch(_e){}
            // Pulisci eventuale hash legacy (#carica / #upload / ecc.)
          ...
          try{ window.scrollTo({ top:0, left:0, behavior:"smooth" }); }catch(_e){ try{ window.scrollTo(0,0); }catch(_e2){} }
        });
      }catch(_e){}
      ...
      function ji() {
        ...
        // ✅ Se c’è un job corrente, interrompilo (evita update tardivi e risultati mescolati)
        try{
          const jid = N.payroll?.currentJobId || "";
          const job = jid ? (N.payroll?.jobs?.[jid] || null) : null;
          if(job?.xhr && typeof job.xhr.abort === "function") job.xhr.abort();
          if(job) job.xhr = null;
        }catch(_e){}

        // ✅ Se c’è un’estrazione/upload in corso, interrompi (fallback legacy)
        try{ if(e.__extractXhr){ e.__extractXhr.abort(); } }catch(_e){}
        try{ e.__extractXhr = null;
              try{ stopExtractEtaTicker(); }catch(_e){}
              try{ e.__extractPhase = ""; }catch(_e){} }catch(_e){}

        // ✅ Job: nessun job attivo (mantieni lo storico in state.payroll.jobs)
        try{ if(N.payroll) N.payroll.currentJobId = ""; }catch(_e){}

        // ✅ Reset cache PDF (split veloce)
        try{ e.__pdfSrcBytesRef = null; e.__pdfSrcDoc = null; }catch(_e){}

        // ✅ Reset promesse/async state
        try{ e.originalPdfBytesPromise = null; }catch(_e){}
        try{ e.sourceFileHashPromise = null; }catch(_e){}
        try{ e.totalPagesPromise = null; }catch(_e){}

        // UX: stop ticker + reset testo stima
        try{ stopExtractEtaTicker(); }catch(_e){}
        try{ const m = U("payrollProgressMetaExtract"); if(m) m.textContent = ""; }catch(_e){}
      ...
      function extractCompanyFromText(rawText){
        const t = String(rawText||"").replace(/\r/g,"");
        if(!t) return "";
        const lines = t.split("\n").map(l=>l.trim()).filter(Boolean);
        const re = /(ragione\s+sociale|azienda|ditta|datore\s+di\s+lavoro|denominazione|societ[aà]|soc\.)\s*[:\-]\s*(.+)/i;
        for(const line of lines){
          const m = line.match(re);
          if(m && m[2]){
            const cand = normalizeCompanyName(m[2]);
      ...
      // --- Debug helpers (abilita con ?debug=1 o localStorage ilp_debug=1) ---
      function ilpDebugOn(){
        try{
          const qs = (location && location.search) ? location.search : "";
          if(/debug=1/.test(qs) || /ilp_debug=1/.test(qs) || /payroll_debug=1/.test(qs)) return true;
          return localStorage.getItem("ilp_debug")==="1" || localStorage.getItem("payroll_debug")==="1";
        }catch(_e){ return false; }
      }
      function ilpDbg(...args){ if(ilpDebugOn()) try{ console.log(...args); }catch(_e){} }
      function ilpDbgTable(data){ if(ilpDebugOn()) try{ console.table(data); }catch(_e){} }
      ...
      function isValidEmail(v){
        ...
        return /^[^\s@]+@[^\s@]+\.[^\s@]{2,}$/.test(s);
            ...
            // === Cache email dipendenti (Match) — SOLO Firebase (2026-02) ===
      // Richiesta: le email inserite in Match NON devono essere salvate in locale.
      // Vengono salvate esclusivamente su Firestore (payrollDirectory).
      // Nota: la chiave localStorage resta solo per retro-compatibilità, ma non viene più usata.
      ...
      // Legacy helpers (NO-OP): niente lettura/scrittura locale
      function payrollReadEmailCache(){ return {}; }
      function payrollWriteEmailCache(_cache){ /* no-op */ }
      function payrollCacheEmailForNext(_row, _emailLower){ /* no-op */ }
      function payrollGetCachedEmail(_cache, _opts = {}){ return ""; }
...
function __payrollLoadScript(src){
      ...
      async function xi(e, t) {
        ...
        // ✅ Performance: evita di ricaricare/parsingare l’intero PDF ad ogni split.
        // Riusa (quando possibile) il PDFDocument sorgente per lo stesso file.
        let src = null;
        try{
          const a = N?.payroll?.admin;
          if(a){
            if(a.__pdfSrcBytesRef !== e){
              a.__pdfSrcBytesRef = e;
              a.__pdfSrcDoc = null;
            }
            if(a.__pdfSrcDoc) src = a.__pdfSrcDoc;
          }
        }catch(_e){}

        if(!src){
          src = await PDFDocument.load(e);
          try{
            const a = N?.payroll?.admin;
            if(a && a.__pdfSrcBytesRef === e){
              a.__pdfSrcDoc = src;
            }
          }catch(_e){}
        }
      ...
      function refreshGroupedValidation(){
        ...
        const hardBlock = (unassignedPages.length > 0) || (conflictPages.length > 0);
        if(btn){
          btn.disabled = hardBlock || !(readyRows.length);
          // Tooltip (desktop): spiega perché è bloccato
          if(hardBlock){
            const why = conflictPages.length
              ? `Invio bloccato: pagine in conflitto (${conflictPages.map(p=>p+1).join(", ")}). Riesegui l’estrazione.`
              : `Invio bloccato: pagine non assegnate (${unassignedPages.map(p=>p+1).join(", ")}). Riesegui l’estrazione.`;
            btn.title = why;
          }else{
            btn.title = "";
          }
        ...
        if(summary){ summary.textContent = `${rows.length} destinatari · ${totalPages} pagine`; }
        if(matchCount){ matchCount.textContent = `${filteredRows.length} destinatari · ${filteredPages} pagine`; }

                if(matchResultsTitle){
          const employees = filteredRows.length;

          // Mese
          const monthKeys = filteredRows.map(r=>normalizeMonthKey(r.monthKey||"")).filter(Boolean);
          const uniqMonths = Array.from(new Set(monthKeys));
          let monthValue = "—";
          if(uniqMonths.length === 1){
            monthValue = formatPayrollMonthLabel(uniqMonths[0]);
          }else if(uniqMonths.length > 1){
            monthValue = "Mesi diversi";
          }

          // Azienda (estratta dal PDF, se presente)
          const companyList = filteredRows.map(r=>normalizeCompanyName(r.companyName||"")).filter(Boolean);
          const uniqCompaniesNorm = Array.from(new Set(companyList.map(v=>v.toLowerCase())));
          const companyLabel = (uniqCompaniesNorm.length===1)
            ? (companyList.find(v=>v && v.toLowerCase()===uniqCompaniesNorm[0]) || "")
            : "";

          const companyCodes = filteredRows.map(r=>String(r.companyCode||"").replace(/\s+/g,"").trim()).filter(Boolean);
          const uniqCompanyCodes = Array.from(new Set(companyCodes));
          const companyCodeLabel = (uniqCompanyCodes.length===1) ? uniqCompanyCodes[0] : "";

          let companyValue = "Azienda non rilevata";
          if(uniqCompaniesNorm.length > 1){
            companyValue = "Aziende diverse";
          }else if(companyLabel){
            companyValue = companyLabel;
          }
          const companyMeta = companyCodeLabel ? `cod. ${companyCodeLabel}` : "";
          const companyMetaHtml = companyMeta ? (' <span class="mcMeta">' + Ao(companyMeta) + '</span>') : "";

          // Totale da pagare (somma dei netti rilevati; se alcuni importi mancano, mostriamo “+”)
          function __ilpParseEuroToCents(txt){
            try{
              ...
              if(s.includes(",")){
                ...
                if(parts.length > 2){
                  ...
                  const dec = parts[1] || "";
                  // Se sembra separatore migliaia (es. 1.234), unisci
                  if(dec.length === 3) norm = parts.join("");
                  else norm = s;
                }else{
                  norm = s;
                }
          ...
          for(const r of filteredRows){
            let cents = null;
            if(typeof r?.netPayCents === "number" && Number.isFinite(r.netPayCents)){
              cents = Math.round(r.netPayCents);
            }else{
              cents = __ilpParseEuroToCents(r?.amountText || r?.netPayText || "");
            }
          ...
          const totalPay = anyAmount ? (formatEuroFromCents(totalCents) + (missingAmount ? " +" : "")) : "—";

          matchResultsTitle.innerHTML = `
            <div class="matchCockpit" role="group" aria-label="Riepilogo invio">
              <div class="mcBox">
                <div class="mcLabel">Azienda</div>
                <div class="mcValue" title="${Ao(companyValue)}">${Ao(companyValue)}${companyMetaHtml}</div>
              </div>
              <div class="mcBox">
                <div class="mcLabel">Mese</div>
                <div class="mcValue" title="${Ao(monthValue)}">${Ao(monthValue)}</div>
              </div>
              <div class="mcBox">
                <div class="mcLabel">Totale da pagare</div>
                <div class="mcValue big" title="${Ao(totalPay)}">${Ao(totalPay)}</div>
              </div>
              <div class="mcBox">
                <div class="mcLabel">Totale dipendenti</div>
                <div class="mcValue" title="${Ao(String(employees))}">${Ao(String(employees))}</div>
              </div>
            </div>
          `;
        }
      ...
      function updateSentToggleUI(){
        ...
        if(sentCount <= 0){
          btn.disabled = true;
          btn.textContent = "Mostra già inviate";
      ...
      function buildGroupedRows(){
        ...
        const items = (docs && docs.length) ? docs : pages.map(p=>({
          ...
          companyName: p.fields?.companyName || "",
          companyCode: p.fields?.companyCode || "",
          hireDate: p.fields?.hireDate || ""
        }));
        ...
        items.forEach((item, idx)=>{
          ...
          const companyName = normalizeCompanyName(item.companyName || fields.companyName || "");
          const companyCode = String(item.companyCode || fields.companyCode || "").replace(/\s+/g,"").trim();
          const hireDate = String(item.hireDate || fields.hireDate || "").trim();
          ...
          if(!row){
            ...
            row = {
              ...
              companyCode: prev?.companyCode || companyCode || "",
              hireDate: prev?.hireDate || hireDate || "",
          ...
          if(!row.companyCode && companyCode) row.companyCode = companyCode;
          if(!row.hireDate && hireDate) row.hireDate = hireDate;
        ...
        const rows = Array.from(groups.values()).map(r=>{
          ...
          const rawPages = Array.from(new Set(r.pages.map(p=>p.pageIndex))).sort((a,b)=>a-b);
          const hadConflictPages = rawPages.some(pi=>conflictPages.has(pi));
          const uniquePages = rawPages.filter(pi=>!conflictPages.has(pi));
          ...
          const companyValues = Array.from(new Set((r.sources||[]).map(src=>normalizeCompanyName(src.companyName || src.fields?.companyName || "")))).filter(Boolean);
          if(!r.companyName && companyValues[0]) r.companyName = companyValues[0];
          const companyCodeValues = Array.from(new Set((r.sources||[]).map(src=>String(src.companyCode || src.fields?.companyCode || "").replace(/\s+/g,"").trim()))).filter(Boolean);
          if(!r.companyCode && companyCodeValues[0]) r.companyCode = companyCodeValues[0];
          const hireDateValues = Array.from(new Set((r.sources||[]).map(src=>String(src.hireDate || src.fields?.hireDate || "").trim()))).filter(Boolean);
          if(!r.hireDate && hireDateValues[0]) r.hireDate = hireDateValues[0];
          ...
          r.conflict = nameCollision || (!fiscalNorm && r.dirCandidatesCount > 1) || r.conflict || hadConflictPages || uniquePages.length===0;
          r.missingMonth = !r.monthKey;
          r.missingEmail = !isValidEmail(r.email);
          return r;
        });
        ...
        if(ilpDebugOn()){
          try{
            window.__ILP_DEBUG = window.__ILP_DEBUG || {};
            window.__ILP_DEBUG.groupedRows = rows;
            window.__ILP_DEBUG.match = N.payroll.admin.match;
          }catch(_e){}
        }
        Pi();
      }
      ...
      async function openPayrollPages(key){
        try{
          ...
          const admin = N.payroll?.admin;
          let srcBytes = admin?.originalPdfBytes || null;

          // Se l’utente clicca subito (prima che il browser finisca di leggere il PDF), aspetta il buffer.
          if(!srcBytes && admin?.originalPdfBytesPromise){
            try{
              Ve("Preparazione…", "Sto preparando il PDF per l’anteprima.");
              srcBytes = await admin.originalPdfBytesPromise;
              try{ admin.originalPdfBytes = srcBytes; }catch(_e){}
            }catch(_e){}
          }
          if(!srcBytes) throw new Error("PDF origine non pronto. Attendi qualche secondo e riprova.");

          const pages = (row.pages||[]).map(p=>p.pageIndex);
          if(!pages.length) throw new Error("Nessuna pagina assegnata.");

          const bytes = await xi(srcBytes, pages);
...
async function saveDirectoryEntry() {
        ...
        try {
          await N.firebase.api.setDoc(N.firebase.api.doc(N.firebase.db,COL_PAYROLL_DIRECTORY, payrollDirDocId(emailLower)), payload, { merge: !0 });
      ...
      async function deleteDirectoryEntry(emailLower){
        ...
        try{
          await N.firebase.api.deleteDoc(N.firebase.api.doc(N.firebase.db, COL_PAYROLL_DIRECTORY, payrollDirDocId(emailLower)));
          await zi();
        }catch(err){
...
function Pi(){
  ...
  if(html){
        ...
        <tr>
          <td colspan="6" style="padding:18px 14px;text-align:center">
            ...
            <div style="text-align:center">
              <div class="pcTitle" style="font-size:16px;margin:0;color:#0b5d2a">✅ Operazione completata: tutte le buste paga risultano inviate.</div>
              <div class="pcSub" style="margin-top:6px;color:rgba(15,23,42,.62)">Puoi chiudere questa schermata o procedere con un nuovo caricamento.</div>
            </div>
          </div>
        </td>
      </tr>
    `;
      ...
      body.innerHTML = '<tr><td colspan="6" class="muted" style="padding:14px">Nessun destinatario.</td></tr>';
    }
  }
  if(counter) counter.textContent = `${filtered.length} destinatari`;
...
function renderUploadHistory(){
      ...
      async function startPayrollDirectoryWatch(){
        ...
        if(!(N.firebase?.ok && N.user?.uid)) return;
        const api = N.firebase.api;
        const db = N.firebase.db;
        const q = api.query(api.collection(db, COL_PAYROLL_DIRECTORY), api.where("ownerUid", "==", N.user.uid));
        N.payroll.directory.unsub = api.onSnapshot(q, snap=>{
          const entries = [];
          snap?.forEach(doc=>{ const d = doc.data() || {}; if(d.enabled === false) return; d.id = doc.id; d.emailLower = (d.emailLower || d.email || "").toLowerCase(); entries.push(d); });
      ...
      async function zi() {
        ...
        try {
          ...
          snap.forEach(doc => { 
            ...
            d.id = doc.id; 
            d.emailLower = (d.emailLower || d.email || "").toLowerCase(); 
            entries.push(d); 
          });
          entries.sort((a,b)=> (a.displayName||a.emailLower||"").localeCompare(b.displayName||b.emailLower||""));
      ...
      // ===== UX: progress “parlante” (stima tempi realistica) =====
      // Baseline (misurazioni reali): ~29s per 10 pagine e ~37s per 19 pagine.
      // Modello semplice: ~20s di overhead + ~0,9s/pagina (valori di default qui sotto).
      // Override (opzionale):
      //   globalThis.ILP_PAYROLL_EST_BASE_SEC = 20
      //   globalThis.ILP_PAYROLL_EST_SEC_PER_PAGE = 0.9
      function __ilpClamp(n, a, b){ n = Number(n)||0; return Math.max(a, Math.min(b, n)); }

      function __ilpFmtDuration(sec){
      ...
      function __ilpEstimateAnalysisSec(pages){
        const p = Math.max(0, Math.round(Number(pages)||0));
        if(!p) return 0;
        const base = ("ILP_PAYROLL_EST_BASE_SEC" in globalThis) ? Number(globalThis.ILP_PAYROLL_EST_BASE_SEC) : 20;
        const perPage = ("ILP_PAYROLL_EST_SEC_PER_PAGE" in globalThis) ? Number(globalThis.ILP_PAYROLL_EST_SEC_PER_PAGE) : 0.9;
        const total = (base + perPage * p);
        return Math.max(4, Math.round(__ilpClamp(total, 4, 12*60)));
      }

      function __ilpFadeSetText(el, text){
        try{
          if(!el) return;
          const next = String(text || "");
          if(el.textContent === next) return;
          el.style.opacity = "0";
          try{ if(el.__fadeTimer) clearTimeout(el.__fadeTimer); }catch(_e){}
          el.__fadeTimer = setTimeout(()=>{
            try{ el.textContent = next; }catch(_e){}
            try{ el.style.opacity = "1"; }catch(_e){}
          }, 420);
        }catch(_e){
          try{ if(el) el.textContent = String(text||""); }catch(_e2){}
        }
      }

      function setExtractMetaFast(text){
        const el = U("payrollProgressMetaExtract");
        if(!el) return;
        const next = String(text || "");
        if(el.textContent === next) return;
        el.textContent = next;
      ...
      function setExtractMetaFade(text){
        __ilpFadeSetText(U("payrollProgressMetaExtract"), text);
      }

      function stopExtractEtaTicker(){
        try{
          const a = N.payroll?.admin;
          if(!a) return;
          const s = a.__extractEta;
          if(s){
            try{ if(s.timer) clearInterval(s.timer); }catch(_e){}
            try{ if(s.tInit) clearTimeout(s.tInit); }catch(_e){}
          }
          a.__extractEta = null;
        }catch(_e){}
      }

      function ensurePayrollJobSystem(){
        try{
          N.payroll = N.payroll || {};
          if(!N.payroll.jobs) N.payroll.jobs = {};
          if(typeof N.payroll.currentJobId !== "string") N.payroll.currentJobId = "";
        }catch(_e){}
      }

      function payrollCreateJob(file){
        ensurePayrollJobSystem();

        // Se c’è un job attivo con XHR in corso, interrompilo: l’utente sta partendo con un nuovo upload
        const prevId = N.payroll?.currentJobId || "";
        try{
          const prevJob = prevId ? (N.payroll?.jobs?.[prevId] || null) : null;
          if(prevJob?.xhr && typeof prevJob.xhr.abort === "function") prevJob.xhr.abort();
        }catch(_e){}
        try{
          const a = N.payroll?.admin;
          if(a?.__extractXhr && typeof a.__extractXhr.abort === "function") a.__extractXhr.abort();
        }catch(_e){}
        try{ stopExtractEtaTicker(); }catch(_e){}

        const jobId = (globalThis.crypto && typeof crypto.randomUUID === "function")
          ? crypto.randomUUID()
          : (String(Date.now()) + "_" + Math.random().toString(16).slice(2));

        try{
          N.payroll.jobs[jobId] = {
            id: jobId,
            file: file || null,
        ...
        try{ N.payroll.currentJobId = jobId; }catch(_e){}
        return jobId;
      }

      function getPayrollJob(jobId){
        try{ return (jobId && N.payroll?.jobs) ? N.payroll.jobs[jobId] : null; }catch(_e){ return null; }
      }

      function isPayrollJobActive(jobId){
        try{ return !!jobId && N.payroll?.currentJobId === jobId; }catch(_e){ return false; }
      }

      function payrollSetJobProgress(jobId, pct, label){
        if(jobId !== N.payroll?.currentJobId) return;
        const p = Math.max(0, Math.min(100, Number(pct)||0));
        try{
          const job = getPayrollJob(jobId);
          if(job) job.progress = { pct: p, label: String(label||"") };
        }catch(_e){}
        try{ setProgress(p, label); }catch(_e){}
      }

      function payrollSetJobMetaFast(jobId, text){
        if(jobId !== N.payroll?.currentJobId) return;
        try{ setExtractMetaFast(text); }catch(_e){}
      }

      function payrollSetJobMetaFade(jobId, text){
        if(jobId !== N.payroll?.currentJobId) return;
        try{ setExtractMetaFade(text); }catch(_e){}
      }

      function startExtractEtaTicker(jobId, pagesPromise){
        try{
          const a = N.payroll?.admin;
          if(!a) return;
          stopExtractEtaTicker();
          a.__extractEta = { jobId, startedAt: performance.now(), pages: 0, totalSec: 0, timer: null, tInit: null };

          const st = a.__extractEta;

          // best-effort: recupera #pagine e stima tempo totale (non blocca)
          try{ if(jobId === N.payroll?.currentJobId) setExtractMetaFast("Analisi in corso…"); }catch(_e){}
          if(pagesPromise && typeof pagesPromise.then === "function"){
            st.tInit = setTimeout(()=>{
              try{
                pagesPromise.then((pc)=>{
                  try{
                    if(jobId !== N.payroll?.currentJobId) return;
                    if(a.__extractPhase !== "analysis") return;
                    const pages = Math.max(0, Math.round(Number(pc)||0));
                    if(pages > 0){
                      st.pages = pages;
                      st.totalSec = __ilpEstimateAnalysisSec(pages);
                      setExtractMetaFade(`Stima: ~${__ilpFmtDuration(st.totalSec)} · ${pages} pagine`);
                    }
                  }catch(_e){}
                }).catch(()=>{});
              }catch(_e){}
          ...
          const tick = ()=>{
            try{
              if(jobId !== N.payroll?.currentJobId) return stopExtractEtaTicker();
              if(N.payroll?.admin?.step !== "extracting") return stopExtractEtaTicker();
              if(a.__extractPhase !== "analysis") return;

              const elapsed = (performance.now() - st.startedAt) / 1000;
              if(st.totalSec > 0){
                const rem = st.totalSec - elapsed;
                const remSec = Math.max(0, Math.ceil(rem));
                if(remSec > 1){
                  setExtractMetaFast(`Stima: ~${__ilpFmtDuration(remSec)} rimanenti · ${st.pages || "?"} pagine`);
                }else{
                  setExtractMetaFast("Stima: ultimi secondi…");
                }
              }else{
                setExtractMetaFast("Analisi in corso…");
              }
            }catch(_e){}
          };
...
function setProgress(pct, label){
        ...
        if(lbl && label) __ilpFadeSetText(lbl, label);
        if(lblExtract && label) __ilpFadeSetText(lblExtract, label);
      }

      async function Vi(jobId) {
        const e = N.payroll.admin, t = U("payrollIdleHint"), dbg = U("payrollGeminiDebug");

        ensurePayrollJobSystem();

        // Fallback: se chiamato senza jobId (vecchi handler), creane uno dal file selezionato
        try{
          if(!jobId){
            const f0 = e.files?.[0];
            if(f0) jobId = payrollCreateJob(f0);
          }
        }catch(_e){}

        let __job = getPayrollJob(jobId);
        try{ if(__job && !__job.file && e.files?.[0]) __job.file = e.files[0]; }catch(_e){}
        const __file = __job?.file;

        if (!__file) return void(dbg && (dbg.textContent = "Seleziona un PDF per iniziare"));

        // ✅ SOLO il job attivo può scrivere stato/UI
        if(jobId !== N.payroll.currentJobId) return;

        // Gate premium (sicurezza): evita bypass su incognito / device nuovi
        if(!ensurePayrollCanUpload({ toast:true, openBilling:true })){
          try{ payrollSetJobProgress(jobId, 0, "Bloccato"); }catch(_e){}
          return;
        }

        const t0 = performance.now();
        payrollSetJobProgress(jobId, 5, "Preparazione…");

        // Token del job: usalo anche come clientToken (dedup/log lato server)
        const __token = String(__job?.token || jobId || "");
        try{ if(__job && !__job.token) __job.token = __token; }catch(_e){}
        try{
          if(__job){
            __job.error = "";
            __job.result = null;
        ...
        // Se c’era un XHR in corso (job precedente), abort
        try{ if(e.__extractXhr) e.__extractXhr.abort(); }catch(_e){}
        try{ e.__extractXhr = null; }catch(_e){}

        try {
          const file = __file;
          e.sourceFileName = file.name || "documento.pdf";

          const isPdf = (String(file.type||"").toLowerCase().includes("pdf") || String(e.sourceFileName||"").toLowerCase().endsWith(".pdf"));
          if (!isPdf){
            try{ Ve("Formato non valido", "Carica un file PDF."); }catch(_e){}
            try{ payrollSetJobProgress(jobId, 0, "Errore formato"); }catch(_e){}
            return;
          }

          // Stato UI: pulisci risultati precedenti (solo job attivo)
          if(jobId !== N.payroll.currentJobId) return;
          try{
            e.gemini = e.gemini || { loading: !1, error: "", pages: [] };
            e.gemini.error = "";
            e.gemini.pages = [];
            e.gemini.docs = [];
          }catch(_e){}
          try{ e.groupedRows = []; }catch(_e){}
          try{ e.match = { pages: [], grouped: [], unmatched: [], matchedCount: 0, ambiguousCount: 0, unmatchedCount: 0 }; }catch(_e){}
          try{ e.previewRowKey = ""; }catch(_e){}
          try{ e.sendSummary = null; }catch(_e){}

          e.gemini.loading = !0;
          Di("extracting");

          // UX: reset testi e stato stima
          try{ stopExtractEtaTicker(); }catch(_e){}
          try{ e.__extractPhase = "upload"; }catch(_e){}
          try{ payrollSetJobMetaFade(jobId, "1/3 Caricamento PDF: preparazione…"); }catch(_e){}

          // Warm-up libreria PDF in background (non blocca il caricamento)
          try{ __payrollGetPDFLib(); }catch(_e){}

          // Prepara lettura bytes in background (serve per anteprima / invio) — NON blocca l’upload
          try{ e.originalPdfBytes = null; }catch(_e){}
          try{ e.sourceFileHash = ""; }catch(_e){}
          try{
            e.originalPdfBytesPromise = (async ()=>{
              const buf = await file.arrayBuffer();
              // ✅ Non sovrascrivere se nel frattempo è partito un nuovo job
              if(jobId !== N.payroll.currentJobId) return buf;
              try{ e.originalPdfBytes = buf; }catch(_e){}
              return buf;
            })();
          }catch(_e){
            try{ e.originalPdfBytesPromise = null; }catch(_e2){}
          }

          // Conteggio pagine (best-effort): serve solo per mostrare una stima tempi (non blocca l’upload)
          try{
            e.__pageCount = 0;
            e.totalPagesPromise = (async ()=>{
              try{
                const buf = await (e.originalPdfBytesPromise || file.arrayBuffer());
                const lib = await __payrollGetPDFLib();
                const doc = await lib.PDFDocument.load(buf);
                const pages = (doc && typeof doc.getPageCount === "function") ? doc.getPageCount() : 0;
                return pages || 0;
              }catch(_e){
                return 0;
              }
            ...
            // Salva il numero pagine appena disponibile (per UI stima)
            e.totalPagesPromise.then((pc)=>{
              if(jobId !== N.payroll.currentJobId) return;
              try{
                const n = Math.max(0, Math.round(Number(pc)||0));
                e.__pageCount = n;
              }catch(_e){}
            }).catch(()=>{});
          }catch(_e){
            try{ e.totalPagesPromise = null; }catch(_e2){}
          }

          // Hash (best-effort) fuori dal percorso critico
          try{
            e.sourceFileHashPromise = (async ()=>{
              try{
                const buf = await (e.originalPdfBytesPromise || file.arrayBuffer());
                const hash = await crypto.subtle.digest("SHA-256", buf);
                const hex = Array.from(new Uint8Array(hash)).map(b => b.toString(16).padStart(2, "0")).join("");
                // ✅ Non sovrascrivere se nel frattempo è partito un nuovo job
                if(jobId !== N.payroll.currentJobId) return hex;
                try{ e.sourceFileHash = hex; }catch(_e){}
                return hex;
              }catch(_e){ return ""; }
            })();
          }catch(_e){}
          ...
          const fmtMoney = (val)=>{
            ...
            const o = (n / 100).toFixed(2).replace(".", ",");
            const [i, a] = o.split(",");
            const r = i.replace(/\B(?=(\d{3})+(?!\d))/g, ".");
            return `${r},${a}`;
          };

          // ✅ Guard jobId: SOLO il job attivo può aggiornare UI/progress
          const isCurrent = ()=>{ try{ return jobId === N.payroll.currentJobId; }catch(_e){ return false; } };
          const safeProgress = (pct, label)=>{ try{ payrollSetJobProgress(jobId, pct, label); }catch(_e){} };
          const safeMetaFast = (txt)=>{ try{ payrollSetJobMetaFast(jobId, txt); }catch(_e){} };
          const safeMetaFade = (txt)=>{ try{ payrollSetJobMetaFade(jobId, txt); }catch(_e){} };
          const sleep = (ms)=> new Promise(res=> setTimeout(res, Math.max(0, Number(ms)||0)));

          // Build FormData fresh per ogni tentativo (alcuni browser non amano riusare lo stesso FormData su retry)
          const buildExtractFormData = ()=>{
            const fd = new FormData();
            fd.append("file", file, file.name || "documento.pdf");
            // Campi extra (ignorati dal backend se non servono) utili per log/dedup
            try{ fd.append("clientToken", __token); }catch(_e){}
            try{ fd.append("jobId", String(jobId || "")); }catch(_e){}
            try{ fd.append("sourceFileName", String(e.sourceFileName || file.name || "documento.pdf")); }catch(_e){}
            try{ if(e.sourceFileHash) fd.append("sourceFileHash", String(e.sourceFileHash)); }catch(_e){}
            return fd;
          };

          const uploadWithProgress = (fd)=> new Promise((resolve,reject)=>{
            try{
              if(jobId !== N.payroll.currentJobId) return resolve("");

              const xhr = new XMLHttpRequest();

              // Se nel frattempo è partita un’altra estrazione, non iniziare nemmeno
              if(jobId !== N.payroll.currentJobId){
                try{ xhr.abort(); }catch(_e){}
                return resolve("");
              }

              // Registra XHR nel job (e nel fallback legacy) SOLO se è ancora corrente
              try{
                if(jobId === N.payroll.currentJobId){
                  e.__extractXhr = xhr;
                  const j = getPayrollJob(jobId);
                  if(j) j.xhr = xhr;
                }
              }catch(_e){}
              ...
              xhr.timeout = 1000 * 60 * 8; // 8 minuti

              let uploadDone = false;
              const up = { start: performance.now(), lastTs: performance.now(), lastLoaded: 0, emaBps: 0 };
              const alpha = 0.22;
              ...
              const computeEtaSec = (loaded, total)=>{
                try{
                  const now = performance.now();
                  const dt = Math.max(0.001, (now - up.lastTs) / 1000);
                  const dl = Math.max(0, (loaded - up.lastLoaded));
                  // aggiorna EMA ogni ~250ms o se avanzamento significativo
                  if(dl > 0 || (now - up.lastTs) > 250){
                    const inst = dl / dt;
                    if(inst > 0){ up.emaBps = up.emaBps ? (up.emaBps*(1-alpha) + inst*alpha) : inst; }
                    up.lastTs = now;
                    up.lastLoaded = loaded;
                  }
                  if(!total || !loaded || !up.emaBps) return 0;
                  return Math.max(0, (total - loaded) / up.emaBps);
                }catch(_e){ return 0; }
              };
              ...
              xhr.upload.onprogress = (evt)=>{
                try{
                  ...
                  if(evt && evt.lengthComputable){
                    ...
                    const p = Math.round(8 + ratio * 52); // 8% -> 60%
                    safeProgress(Math.max(8, Math.min(60, p)), "1/3 Caricamento PDF…");
                    try{
                      const pct2 = evt.total ? Math.round((evt.loaded / evt.total) * 100) : 0;
                      const eta = computeEtaSec(evt.loaded, evt.total);
                      const pagesTxt = (e.__pageCount && e.__pageCount>0) ? ` · ${e.__pageCount} pagine` : "";
                      const etaTxt = (eta && isFinite(eta) && eta>1) ? ` · ~${__ilpFmtDuration(eta)} rimanenti` : "";
                      safeMetaFast(`Caricamento PDF: ${pct2}%${etaTxt}${pagesTxt}`);
                    }catch(_e){}
                  }else{
                    safeProgress(25, "1/3 Caricamento PDF…");
                    try{
                      const pagesTxt = (e.__pageCount && e.__pageCount>0) ? ` · ${e.__pageCount} pagine` : "";
                      safeMetaFast(`Caricamento PDF in corso${pagesTxt}`);
                    }catch(_e){}
                  }
                }catch(_e){}
              };
              ...
              xhr.upload.onload = ()=>{
                try{
                  if(!stillCurrent()) return;
                  uploadDone = true;
                  safeProgress(62, "2/3 Analisi PDF…");
                  try{ e.__extractPhase = "analysis"; }catch(_e){}
                  // Avvia la stima SOLO se è ancora il job corrente
                  try{ if(isCurrent()) startExtractEtaTicker(jobId, e.totalPagesPromise); }catch(_e){}
                }catch(_e){}
              };

              // Durante la risposta (server processing) teniamo vivo il feedback
              xhr.onprogress = ()=>{
                try{
                  if(!stillCurrent()) return;
                  if(uploadDone) safeProgress(80, "2/3 Analisi PDF…");
                }catch(_e){}
              };
              ...
              xhr.onload = ()=>{
                try{
                  ...
                  if(status >= 200 && status < 300) return resolve(xhr.responseText || "");
                  const err = new Error(`HTTP ${status}`);
                  try{ err.status = status; }catch(_e){}
                  try{ err.responseText = xhr.responseText || ""; }catch(_e){}
                  return reject(err);
                }catch(err){ reject(err); }
              };

              xhr.onerror = ()=>{
                try{ if(jobId !== N.payroll.currentJobId) return resolve(""); }catch(_e){}
                reject(new Error("Upload fallito"));
              };
              xhr.ontimeout = ()=>{
                try{ if(jobId !== N.payroll.currentJobId) return resolve(""); }catch(_e){}
                const err = new Error("Timeout durante l'upload");
                try{ err.status = 408; }catch(_e){}
                reject(err);
              };
          ...
          const shouldRetryExtract = (err)=>{
            try{
              ...
              if(msg.includes("upload fallito")) return true;
          ...
          const uploadWithRetry = async ()=>{
            ...
            for(let attempt=1; attempt<=maxAttempts; attempt++){
              if(jobId !== N.payroll.currentJobId) return "";
              if(attempt === 1){
                safeProgress(8, "1/3 Caricamento PDF…");
              }else{
                safeProgress(8, `Riprovo… (${attempt}/${maxAttempts})`);
                safeMetaFade(`Riprovo il caricamento (${attempt}/${maxAttempts})…`);
              }

              try{
                const raw = await uploadWithProgress(buildExtractFormData());
                return raw;
              }catch(err){
                lastErr = err;
                if(jobId !== N.payroll.currentJobId) return "";
                if(attempt < maxAttempts && shouldRetryExtract(err)){
                  // Backoff + jitter (anti “thundering herd” quando siamo in tanti in simultanea)
                  const base = Math.min(12000, 800 * Math.pow(2, attempt-1));
                  const wait = Math.round(base + (base * 0.25 * Math.random()));
                  try{
                    const sec = Math.max(1, Math.ceil(wait/1000));
                    safeMetaFast(`Servizio occupato, ritento tra ~${__ilpFmtDuration(sec)}…`);
                  }catch(_e){}
                  await sleep(wait);
                  continue;
                }
          ...
          const raw = await uploadWithRetry();

          // ✅ Se nel frattempo è partito un nuovo upload/job, non toccare lo stato
          if(jobId !== N.payroll.currentJobId) return;

          try{ stopExtractEtaTicker(); }catch(_e){}
          try{ e.__extractPhase = "organize"; }catch(_e){}
          safeProgress(92, "3/3 Organizzazione risultati…");
          try{ safeMetaFade("Organizzo le buste paga per dipendente e preparo l’elenco…"); }catch(_e){}
          let c;
          try{ c = raw ? JSON.parse(raw) : null; }catch(_e){ throw new Error("Risposta non valida dal servizio di estrazione"); }
          if(!c) throw new Error("Risposta vuota dal servizio di estrazione");

          safeProgress(100, "Analisi completata");
          try{ safeMetaFade("Analisi completata."); }catch(_e){}
          if(isCurrent()) dbg && (dbg.textContent = `Estrazione OK · ${Math.round(performance.now() - t0)}ms`);

          const d = Array.isArray(c.documents) ? c.documents : [];

          // ==== Page index base (0-based vs 1-based) — robust per-file ====
          // Bugfix: alcuni extractor ritornano pageNos 0-based (0..N-1) ma il vecchio codice
          // decideva la base PER documento (se non vedeva lo zero ⇒ assumeva 1-based),
          // causando uno shift (-1) delle pagine dalle buste successive:
          // es. 19 pagine → un dipendente da 3 pagine diventa 2, e poi tutto il resto è sfasato.
          const normalizePageList = (val)=>{ 
            if(Array.isArray(val)) return val; 
            if(typeof val==="number" && Number.isFinite(val)) return [val]; 
            if(typeof val==="string"){ 
              // supporta: "1 2 3" / "1,2,3" / "1;2;3"
              const parts = val.split(/[,;\s]+/).map(v=>Number(v)).filter(v=>Number.isFinite(v)); 
              if(parts.length) return parts; 
            } 
            return []; 
          };

          // Best-effort: # pagine del PDF (serve per scegliere la base migliore)
          let __pdfPageCount = 0;
          try{ __pdfPageCount = Math.max(0, Math.round(Number(e.__pageCount || 0))); }catch(_e){ __pdfPageCount = 0; }
          if(!__pdfPageCount){
            try{
              if(e.totalPagesPromise && typeof e.totalPagesPromise.then === "function"){
                const pc = await Promise.race([
                  e.totalPagesPromise,
                  new Promise(res=>setTimeout(()=>res(0), 1200))
                ]);
                __pdfPageCount = Math.max(0, Math.round(Number(pc||0)));
              }
            }catch(_e){ __pdfPageCount = 0; }
          }

          function __ilpNormIntList(arr){
            return (arr||[]).map(v=>Number(v)).filter(v=>Number.isFinite(v)).map(v=>Math.round(v));
          }
          function __ilpDedupSort(arr){
            try{
              const s = new Set();
              (arr||[]).forEach(v=>{ if(Number.isFinite(v)) s.add(v); });
              return Array.from(s).sort((a,b)=>a-b);
            }catch(_e){
              return [];
            }
          }
          function __ilpConvertPages(rawList, mode){
            let nums = __ilpDedupSort(__ilpNormIntList(rawList));
            if(!nums.length) return [];
            if(mode === "one") nums = nums.map(v=>v-1);
            nums = nums.filter(v=>Number.isFinite(v));
            if(__pdfPageCount > 0){
              nums = nums.filter(v=>v>=0 && v<__pdfPageCount);
            }else{
              nums = nums.filter(v=>v>=0);
            }
            return __ilpDedupSort(nums);
          }

          function __ilpScoreBase(mode){
            const counts = new Map();
            let outOfRange = 0;

            for(let i=0;i<d.length;i++){
              const t = d[i] || {};
              const rawList = normalizePageList(t.pageNos || t.pageIndices || t.pages || t.pageIndex);
              if(!rawList || !rawList.length) continue; // evita bias da fallback
              const pages = __ilpConvertPages(rawList, mode);
              if(!pages.length) continue;
              pages.forEach(pi=>{
                if(__pdfPageCount>0 && (pi<0 || pi>=__pdfPageCount)){
                  outOfRange++;
                  return;
                }
            ...
            let overlaps = 0;
            counts.forEach(c=>{ if(c>1) overlaps += (c-1); });

            const unique = counts.size;
            const missing = (__pdfPageCount>0) ? Math.max(0, __pdfPageCount - unique) : 0;

            // Score: privilegia copertura e penalizza sovrapposizioni / fuori range
            const score = (unique * 1000) - (missing * 25) - (overlaps * 40) - (outOfRange * 250);
            return { score, unique, overlaps, outOfRange, missing };
          }

          // Decide una base UNICA per tutto il file (evita shift tra doc)
          let __pageBase = "one"; // default: 1-based (molti sistemi usano 1..N)
          try{
            const allNums = [];
            for(let i=0;i<d.length;i++){
              const t = d[i] || {};
              ...
              if(rawList && rawList.length){
                allNums.push(...__ilpNormIntList(rawList));
              }
            }
            const hasAny = allNums.some(v=>Number.isFinite(v));
            const hasZero = allNums.some(v=>v===0);

            if(!hasAny){
              // nessuna informazione pagina: base irrilevante (useremo fallback [iDoc])
              __pageBase = "one";
            }else if(__pdfPageCount > 0){
              const s0 = __ilpScoreBase("zero");
              const s1 = __ilpScoreBase("one");
              __pageBase = (s0.score >= s1.score) ? "zero" : "one";
            }else{
              // senza pageCount: fallback semplice
              __pageBase = hasZero ? "zero" : "one";
            }
          }catch(_e){
            __pageBase = "one";
          }
          ...
          for (let iDoc = 0; iDoc < d.length; iDoc++) {
            ...
            const monthRaw = t.documentDate || t.monthKey || t.document_date || t.date || t.month || t.period || t.payPeriod || t.pay_period || t.pay_period || "";
            ...
            if(!netPayText && netPayCents != null) netPayText = fmtMoney(netPayCents);

            let companyName = normalizeCompanyName(
              t.companyName || t.company_name || t.company || t.employerName || t.employer_name || t.employer
              || t.azienda || t.ditta || t.ragioneSociale || t.ragione_sociale || t.societa || t["società"] || t.organization || t.organisation || t.organizzazione || ""
              || t.fields?.companyName || t.fields?.company_name || t.fields?.company || t.fields?.employerName || t.fields?.employer_name || t.fields?.employer
              || t.fields?.azienda || t.fields?.ditta || t.fields?.ragioneSociale || t.fields?.ragione_sociale || ""
              || t.company?.name || t.employer?.name || t.fields?.company?.name || t.fields?.employer?.name || ""
            );
            if(!companyName) companyName = extractCompanyFromText(rawText);

            const companyCode = String(
              t.companyCode || t.company_code || t.codiceAzienda || t.codice_azienda || t.employerCode || t.employer_code || ""
            ).replace(/\s+/g,"").trim();

            const hireDate = String(
              t.hireDate || t.hire_date || t.dataAssunzione || t.data_assunzione || ""
            ).trim();

            const pageIndicesRaw = normalizePageList(t.pageNos || t.pageIndices || t.pages || t.pageIndex);
            let pageIndices = __ilpConvertPages(pageIndicesRaw, __pageBase);
            if(!pageIndices.length && Number.isFinite(t.pageIndex)) pageIndices = __ilpConvertPages([t.pageIndex], __pageBase);

            // Fallback: se il backend non fornisce info pagine, assumiamo "1 doc = 1 pagina" (ordine di risposta)
            if(!pageIndices.length){
              const fallback = Math.max(0, Math.round(Number(iDoc)||0));
              if(!__pdfPageCount || fallback < __pdfPageCount) pageIndices = [fallback];
              else pageIndices = [];
            }
            ...
            const doc = {
              ...
              fields: { fullName, monthKey, fiscalCode, netPayText, netPayCents, companyName, companyCode, hireDate },
              fullName,
              fiscalCode,
              companyName,
              companyCode,
          ...
          // ✅ Prima di scrivere nello stato, ricontrolla il job attivo
          if(jobId !== N.payroll.currentJobId) return;

          e.gemini.docs = docs;
          e.gemini.pages = pagesFlat;

          // Salva nello stato del job (isolato per jobId)
          try{
            if(jobId === N.payroll.currentJobId){
              const j = getPayrollJob(jobId);
              if(j){
                j.result = { raw: c, docs, pages: pagesFlat };
                j.error = "";
              }
            }
          }catch(_e){}

          // DEBUG: mostra in console se l'azienda viene estratta (abilita con ?debug=1 o localStorage ilp_debug=1)
          if(ilpDebugOn()){
            try{
              window.__ILP_DEBUG = window.__ILP_DEBUG || {};
              window.__ILP_DEBUG.lastExtractRaw = c;
              window.__ILP_DEBUG.docs = docs;
              window.__ILP_DEBUG.pages = pagesFlat;

              console.log("[ILP][DEBUG] Estrazione completata:", { docs: docs.length, pages: pagesFlat.length });
            }catch(_e){}
          }

          if(isCurrent()) t && (t.textContent = `PREVIEW (${pagesFlat.length})`);
          if(isCurrent()) dbg && (dbg.textContent = `Estrazione OK · pagine ${pagesFlat.length} · ${Math.round(performance.now() - t0)}ms`);
          safeProgress(100, "Analisi completata");

          buildGroupedRows();

          // Aggancia le righe raggruppate al job
          try{
            if(jobId === N.payroll.currentJobId){
              const j = getPayrollJob(jobId);
              if(j) j.groupedRows = (N.payroll?.admin?.groupedRows || []);
            }
          }catch(_e){}

          Ii();
          Pi();
          (N.payroll?.admin && (N.payroll.admin.lastUploadStep="preview"));
          Di("match");
        } catch (err) {
          // ✅ Se nel frattempo è partito un nuovo upload/job, ignora questo errore
          if(jobId !== N.payroll.currentJobId) return;

          e.gemini.error = err?.message || String(err);

          // Salva errore nel job
          try{
            if(jobId === N.payroll.currentJobId){
              const j = getPayrollJob(jobId);
              if(j) j.error = e.gemini.error;
            }
          }catch(_e){}

          dbg && (dbg.textContent = `Estrazione ERRORE · ${Math.round(performance.now() - t0)}ms\n${e.gemini.error}`);
          try{ stopExtractEtaTicker(); }catch(_e){}
          try{ safeMetaFast("Errore durante l’analisi."); }catch(_e){}
          console.warn("payroll extract error", err);
          Ve("Estrazione fallita", e.gemini.error);
          safeProgress(0, "Errore");
          Di("idle");
        } finally {
          // ✅ Non spegnere "loading" se nel frattempo è partito un altro job
          try{
            if(jobId === N.payroll.currentJobId){
              e.gemini.loading = !1;
              e.__extractXhr = null;
      ...
      function updatePayrollSendSentToggleUI(){
        ...
        if(sentCount <= 0){
          btn.disabled = true;
          btn.textContent = "Mostra inviate";
          btn.setAttribute("aria-pressed","false");
        ...
        btn.textContent = showingSent ? "Mostra da inviare" : `Mostra inviate (${sentCount})`;
      }
              ...
              body.innerHTML = `
                <tr>
                  <td colspan="6" style="padding:18px 14px;text-align:center">
                    ...
                    <div class="pcSub" style="margin-top:6px;color:rgba(15,23,42,.62)">Restano ${plural} da inviare${extra}. Torna all’elenco per inviare le buste paga rimanenti.</div>
                  </td>
                </tr>
              `;
            }
      ...
      function buildPayrollMailContent({ toEmail, fullName, monthKey, netPayText, companyName }){
        const monthLabel = formatPayrollMonthLabel(monthKey);
        const who = String(fullName || "").trim() || String(toEmail || "").trim() || "—";
        const amount = (netPayText && String(netPayText).trim()) ? String(netPayText).trim() : "";
        ...
        const footerText = comp
          ? `Per qualsiasi dubbio, contatta l’Ufficio Paghe di ${comp}.`
          : "Per qualsiasi dubbio, contatta l’Ufficio Paghe.";
...
${footerText}`;
        ...
        const footerHtml = comp
          ? `Per qualsiasi dubbio, contatta l’<b>Ufficio Paghe</b> di ${Ao(comp)}.`
          : `Per qualsiasi dubbio, contatta l’<b>Ufficio Paghe</b>.`;

        const html =
`<p>Ciao ${Ao(who)},</p>
<p>in allegato trovi la tua busta paga relativa a <b>${Ao(monthLabel)}</b>${amount ? " (<b>"+Ao(amount)+"</b>)" : ""}.</p>
<p style="margin-top:14px;color:#6b7280">${footerHtml}</p>`;

        return { subject, text, html };
      }
      async function sendPayrollPdfViaEndpoint({ toEmail, fullName, monthKey, netPayText, pdfBytes, fileName, companyName, replyTo }){
        const rawEndpoint = String(globalThis.PAYROLL_MAIL_ENDPOINT || "").trim();
        if(!rawEndpoint) throw new Error("PAYROLL_MAIL_ENDPOINT non configurato (serve un endpoint mail su Cloud Run/Fn).");

        const email = String(toEmail || "").trim().toLowerCase();
        const cn = String(companyName || "").trim();
        const rt = String(replyTo || "").trim().toLowerCase();
        if(!isValidEmail(email)) throw new Error("Email destinatario non valida.");

        const { subject, text, html } = buildPayrollMailContent({ toEmail: email, fullName, monthKey, netPayText, companyName: cn });

        const safeName = fileName || "busta_paga.pdf";

        // Build FormData fresh for each attempt (alcuni browser non gradiscono riusare lo stesso FormData su retry)
        const buildFd = ()=>{
          const fd = new FormData();
          fd.append("to", email);
          ...
          if(cn) fd.append("companyName", cn);
          if(rt && isValidEmail(rt)) fd.append("replyTo", rt);
        ...
        // 🔧 Fallback endpoint:
        // In alcuni deploy il mailer risponde su POST "/" invece che su POST "/send",
        // oppure passa da Firebase Hosting (rewrite) su "/send".
        const candidates = [];
        const push = (v)=>{
        ...
        // opzionale override runtime
        push(globalThis.PAYROLL_MAIL_ENDPOINT_OVERRIDE);

        // 1) quello configurato (com'è) + variante senza slash finali
        push(rawEndpoint);
        const trimmed = rawEndpoint.replace(/\s+/g,"").replace(/\/+$/,"");
        if(trimmed) push(trimmed);

        // 2) se finisce con /send, prova anche la root; se NON finisce con /send, prova /send
        if(/\/send$/i.test(trimmed)){
          const base = trimmed.replace(/\/send$/i,"");
          push(base);
          push(base + "/");
          push(base + "/send");
          push(base + "/send/");
        ...
        // 3) same-origin (se hai una rewrite in Firebase Hosting)
        push("/send");
        try{ push(location.origin + "/send"); }catch(_e){}

        let lastErr = null;

        for(const endpoint of candidates){
          try{
            // Debug: capisci subito quale URL sta rispondendo
            try{ console.debug("[payroll mail] POST", endpoint); }catch(_e){}

            const res = await fetch(endpoint, { method:"POST", body: buildFd(), headers });

            // 404 = endpoint/route non esistente → prova il prossimo
            if(res.status === 404){
              lastErr = new Error(`Mailer endpoint non trovato (404): ${endpoint}`);
              continue;
            }
            ...
            let j = null;
            try{ j = bodyText ? JSON.parse(bodyText) : null; }catch(_e){ j = null; }
            if(j && j.ok === false) throw new Error(j.error || "Invio non riuscito");
            return j || { ok:true };
          }catch(err){
            lastErr = err;
            // Se è un errore di rete/CORS (TypeError: Failed to fetch), proviamo il prossimo endpoint
            continue;
          }
        }

        throw lastErr || new Error("Invio non riuscito");
      }
...
async function Yi() {
        ...
        if (!e.originalPdfBytes){
            // Se l’utente arriva qui subito dopo l’upload, il browser potrebbe stare ancora leggendo il PDF.
            // Aspetta il buffer (best-effort) invece di bloccare l’operazione.
            if(e.originalPdfBytesPromise){
              try{
                setProgress(10, "Preparo PDF…");
                e.originalPdfBytes = await e.originalPdfBytesPromise;
              }catch(_e){}
            }
          }
        ...
        const _dirKnown = new Set(((N.payroll?.directory?.entries) || [])
          ...
          .filter(Boolean));
        // SAFETY: se il salvataggio email è disattivato e ci sono email "nuove",
        // avvisa prima dell’invio per evitare sorprese al prossimo caricamento.
        try{
          if(!saveEmailsForNext){
            const unknown = (rows || [])
              .map(r=> normalizeEmail(r?.email || ""))
              .filter(em => em && isValidEmail(em) && !_dirKnown.has(em));
            const uniq = Array.from(new Set(unknown));
            if(uniq.length){
              const ok = window.confirm(
                "Salvataggio email disattivato.\n\n" +
                "Se continui, " + uniq.length + " email non verranno memorizzate e dovrai reinserirle al prossimo invio.\n\n" +
        ...
        const _ensureDir = async (row, emailLower) => {
          ...
          try{
            ...
            // UI: indica tentativo salvataggio email (sidebar)
            try{ beginPayrollSaveEmailsSync(); }catch(_e){}
            await N.firebase.api.setDoc(N.firebase.api.doc(N.firebase.db, COL_PAYROLL_DIRECTORY, payrollDirDocId(em)), payload, { merge:true });
            _dirKnown.add(em);
            try{ endPayrollSaveEmailsSync(true); }catch(_e){}
          }catch(err){
            console.warn("auto directory save", err);
            try{ endPayrollSaveEmailsSync(false); }catch(_e){}
            try{
              const em2 = normalizeEmail(emailLower || row?.email || "");
              N.payroll = N.payroll || {};
              N.payroll.admin = N.payroll.admin || {};
              N.payroll.admin.__dirSaveFailed = N.payroll.admin.__dirSaveFailed || {};
              if(em2) N.payroll.admin.__dirSaveFailed[em2] = String(err?.code || err?.message || "save_failed");
            }catch(_e2){}
            ...
            try{
              if(!__dirSaveWarned){
                ...
                const body = (err?.code === "permission-denied" || low.includes("permission"))
                  ? "Permessi insufficienti: impossibile salvare l’email."
                  : "Impossibile salvare in Dipendenti. Riprova.";
...
// === Modalità privacy (NO Storage): invio diretto senza archiviazione ===
        const persistPdfs = payrollShouldPersistUploads();
        if(!persistPdfs){
          Di("sending");
            ...
            for(const row of rows){
              ...
              try{
                ...
                const companyName = String(
                  ...
                  row.employerDisplayName ||
                  row.fields?.companyName ||
                  row.fields?.company ||
                  row.fields?.employerName ||
                  (row.sources && row.sources[0] && (row.sources[0].companyName || row.sources[0].fields?.companyName)) ||
                  globalThis.PAYROLL_DEFAULT_COMPANY_NAME ||
                  ""
                ).trim();
                let replyTo = "";
                try{
                  const map = globalThis.PAYROLL_COMPANY_REPLYTO_MAP;
                  const code = String(row.companyCode || "").trim();
                  if(map && typeof map === "object"){
                    replyTo =
                      (code && (map[code] || map[code.toLowerCase()])) ||
                      (companyName && (map[companyName] || map[companyName.toLowerCase()])) ||
                      "";
                    if(!replyTo && companyName){
                      const key = (typeof normalizeCompanyName === "function"
                ...
                if(!replyTo){
                  try{ replyTo = String(row.replyTo || row.companyReplyTo || "").trim(); }catch(_e){}
                }
        ...
        // Salvataggio PDF (Firebase): richiede utente autenticato con email
        if(!payrollUserCanPersistPdfs()){
          Ve("Salvataggio PDF", "Per salvare i PDF devi essere autenticato con email. Accedi e riprova.");
          try{ openAuthOverlay("Accedi per salvare i PDF su Firebase.", "google"); }catch(_e){}
          return;
        }

        // Assicura Storage (lazy)
        try{
          const okStorage = await ensureFirebaseStorageReady();
          if(!okStorage){
            Ve("Storage", "Firebase Storage non disponibile. Disattiva ‘Salva i PDF’ e riprova.");
            return;
          }
        }catch(err){
          Ve("Storage", err?.message || "Impossibile inizializzare Firebase Storage.");
          return;
        }
        ...
        try {
          ...
          for (const row of rows) {
            ...
            // ✅ Permette la correzione: NON bloccare solo perché il nome file è lo stesso.
            // Skip SOLO se è davvero identico (stesso file + stesso hash + stesse pagine).
            if(__alreadySameFile && sameHash && samePages){
            ...
            // Invio email con PDF allegato (Cloud Run mailer)
            // - Il PDF viene salvato su Firebase (Storage + Firestore)
            // - L’invio mail resta via endpoint (più affidabile e immediato)
            try{
              const pdfBytes = await xi(e.originalPdfBytes, pageIndices);
              const fnameBase = `${row.displayName || row.fiscalCode || emailLower} ${row.monthKey}`.trim();
              const attachmentName = buildSafePdfName(fnameBase);
              ...
              console.warn("payroll email send (endpoint)", err);
              setPayrollSendRowMailStatus(sendKey, "err", err?.message || String(err));
            }
      ...
      async function openAdminModalFlow(targetPane, opts){
        ...
        // Niente route "#carica": la modale è indipendente dall'URL
        const options = opts || {};
      ...
      function openPayrollUploadPicker(){
        ...
        if(!ensurePayrollCanUpload({ toast:true, openBilling:true })) return;
        // Vai alla HOME (nessun #carica) prima di aprire il selettore file
        try{
          try{ navigateSpaToRoute("home"); }catch(_e){}
          const u = new URL(location.href);
          if(u.hash){
      ...
      async function loadPayrollDirectoryAllEmployees(){
        ...
        const api = N.firebase.api, db = N.firebase.db;
        const out = [];
        const snap = await api.getDocs(api.query(api.collection(db, COL_PAYROLL_DIRECTORY), api.where("ownerUid","==", N.user.uid)));
        snap?.forEach(docSnap=>{
          const x = docSnap.data() || {};
          let emailLower = normalizeEmail(x.emailLower || x.email || "");
          if(!emailLower){
            try{
              const id = String(docSnap.id || "");
              const p = id.split("__");
              if(p.length >= 2) emailLower = normalizeEmail(p.slice(1).join("__"));
            }catch(_e){}
          }
      ...
      async function saveSelectedEmployeeEdits(){
        try{
          ...
          await api.setDoc(api.doc(db, COL_PAYROLL_DIRECTORY, payrollDirDocId(newEmail)), payload, { merge:true });

          if(oldEmail && oldEmail !== newEmail){
            try{ await api.deleteDoc(api.doc(db, COL_PAYROLL_DIRECTORY, payrollDirDocId(oldEmail))); }catch(_e){}
          }
      ...
      U("pdfPreviewOverlay")?.addEventListener("click", (evt)=>{
        try{
          const t = evt?.target;
          if(!t) return;
          if(t.closest?.(".pdfPreviewTopbar")) return;
          if(t.closest?.(".pdfPreviewControls")) return;
          if(t.closest?.(".pdfPreviewPaper")) return;
          closePdfPreview();
        }catch(_e){}
      });

      // Bottom controls: pagina / zoom
      U("btnPdfPrevPage")?.addEventListener("click", ()=>{
        try{
          const st = __pdfPreviewGetState();
          __pdfPreviewScrollToPage((st.currentPage||1) - 1, "smooth");
        }catch(_e){}
      });
      U("btnPdfNextPage")?.addEventListener("click", ()=>{
        try{
          const st = __pdfPreviewGetState();
          __pdfPreviewScrollToPage((st.currentPage||1) + 1, "smooth");
        }catch(_e){}
      });
      U("btnPdfZoomOut")?.addEventListener("click", ()=>{
        try{
          const st = __pdfPreviewGetState();
          __pdfPreviewSetZoom((Number(st.zoom||1) - 0.1), { keepPage: st.currentPage||1 });
        }catch(_e){}
      });
      U("btnPdfZoomIn")?.addEventListener("click", ()=>{
        try{
          const st = __pdfPreviewGetState();
          __pdfPreviewSetZoom((Number(st.zoom||1) + 0.1), { keepPage: st.currentPage||1 });
        }catch(_e){}
      });
      U("btnPdfFitWidth")?.addEventListener("click", ()=>{
        try{
          const st = __pdfPreviewGetState();
          __pdfPreviewSetZoom(1, { keepPage: st.currentPage||1 });
        }catch(_e){}
      });
      ...
      U("btnPdfPreviewPrint")?.addEventListener("click", ()=>{
        ...
        // Se siamo in fallback iframe (canvas non disponibile), prova a stampare l'iframe.
        try{
          const isIframeActive = frame && frame.style && frame.style.display !== "none" && frame.src && frame.src !== "about:blank";
          if(isIframeActive && frame.contentWindow){
        ...
        // Default: apri il PDF in una nuova tab e stampa
        if(u){
      ...
      U("btnCloseAdmin")?.addEventListener("click", ()=>{ try{ toggleAdminModal(false); }catch(_e){} });
	      // Fase MATCH: annulla dal menu laterale
	      U("btnPayrollCancelMatch")?.addEventListener("click", ()=>{
        try{ toggleAdminModal(false); }catch(_e){}

        // Root corretto: torna sempre alla HOME (niente #carica/#upload, niente query residue)
        try{
          if(typeof navigateSpaToRoute === "function"){
            navigateSpaToRoute("home");
          }else if(typeof globalThis.ilovepagheSpaNavigate === "function"){
            globalThis.ilovepagheSpaNavigate("home");
          }else if(typeof getSpaUrlForRoute === "function"){
        ...
        // Pulisci eventuale hash legacy (se presente)
        try{
          const u = new URL(location.href);
          if(u.hash){
            u.hash = "";
      ...
      U("adminModal")?.addEventListener("click", evt=>{ if(evt.target===evt.currentTarget){ try{ toggleAdminModal(false); }catch(_e){} } });
...
btnDirSaveEdits && (btnDirSaveEdits.onclick = async ()=>{
  ...
  for(const oldEmail of toEdit){
    ...
    try{
      await N.firebase.api.setDoc(N.firebase.api.doc(N.firebase.db, COL_PAYROLL_DIRECTORY, payrollDirDocId(newEmail)), payload, { merge:true });
      if(newEmail !== oldEmail){
        await N.firebase.api.deleteDoc(N.firebase.api.doc(N.firebase.db, COL_PAYROLL_DIRECTORY, payrollDirDocId(oldEmail)));
      }
    }catch(err){
...
const payrollDirAutofill = () => {
      ...
      const isPdfFile = (file)=>{
        ...
        if(typeof file.type === "string" && file.type.toLowerCase().includes("pdf")) return true;
        const name = String(file.name || "").toLowerCase();
        return name.endsWith(".pdf");
      };
      const isPayrollUploadBusy = ()=> !!(N.payroll?.admin?.gemini?.loading);
      const resetPayrollSelection = ()=>{
        try{ N.payroll.admin.files = []; }catch(_e){}
        try{ Bi(); }catch(_e){}
        try{ if(n) n.value = ""; }catch(_e){}
      };
      const o = (files) => {
        if(isPayrollUploadBusy()){
          try{ Ve("Caricamento in corso", "Attendi la fine dell’upload prima di caricare un altro PDF."); }catch(_e){}
          return;
        }
        ...
        if(!isPdfFile(picked)){
          try{ Ve("Formato non valido", "Carica un file PDF."); }catch(_e){}
          try{ setProgress(0, "Errore formato"); }catch(_e){}
        ...
        try{
          // Nessuna route "#carica": ripulisci eventuale hash legacy
          const u = new URL(location.href);
          if(u.hash === "#carica" || u.hash === "#upload"){
            u.hash = "";
            history.replaceState({ ...(history.state||{}), spa:true, route:"home" }, "", u.href);
          }
        }catch(_e){}
        ...
        const jobId = payrollCreateJob(picked);
        Vi(jobId);
      };
      const isPdfDrag = (evt)=>{
        ...
        if(!hasFileItem) return null;
        const pdfItem = items.find(item=>item.kind === "file" && String(item.type || "").toLowerCase().includes("pdf"));
        if(pdfItem) return true;
        const nonPdf = items.some(item=>item.kind === "file" && item.type);
        if(nonPdf) return false;
        return null;
      };
      t && (["dragover", "dragenter"].forEach(e => t.addEventListener(e, evt => {
        evt.preventDefault();
        evt.stopPropagation();
        if(isPayrollUploadBusy() || t.classList.contains("locked")) return;
        const isPdf = isPdfDrag(evt);
        if(isPdf === false) return;
        t.classList.add("drag");
      })), ["dragleave", "drop"].forEach(e => t.addEventListener(e, evt => { evt.preventDefault(), evt.stopPropagation(), t.classList.remove("drag") })), t.addEventListener("drop", evt => {
        evt.preventDefault();
        evt.stopPropagation();
        if(isPayrollUploadBusy() || t.classList.contains("locked")) return;
        const files = evt.dataTransfer?.files;
        if(files?.length) o(files);
      }), t.addEventListener("click", e => {
        if(e?.target?.closest && e.target.closest(".uploadActions")) return;
        if(isPayrollUploadBusy()){
          try{ Ve("Caricamento in corso", "Attendi la fine dell’upload prima di caricare un altro PDF."); }catch(_e){}
          return;
        }
      ...
      U("btnPayrollUpload")?.addEventListener("click", () => { if(!ensurePayrollCanUpload({ toast:true, openBilling:true })) return; const f = N.payroll.admin.files?.[0]; if (!f) { n?.click(); return; } const jobId = payrollCreateJob(f); Vi(jobId); });
      U("btnPayrollRetry")?.addEventListener("click", () => { const f = N.payroll.admin.files?.[0]; if(!f) return; const jobId = payrollCreateJob(f); Vi(jobId); });
...
// Input live: email match rows + directory edit drafts
      ...
      if(payrollSaveEmailsToggle && !payrollSaveEmailsToggle.__bound){
        ...
        // UI: stato salvataggio email (dot/spinner/✓) accanto allo switch
        try{ ensurePayrollSaveEmailsSyncIndicator(); setPayrollSaveEmailsSyncState("idle", ""); }catch(_e){}
      ...
      // Switch (menu laterale): salvataggio PDF — se manca nell'HTML lo iniettiamo (anti-cache)
      // Nota: non forza lo stato. La logica ON/OFF è gestita dallo switch (localStorage + runtime).
      function ensurePayrollSavePdfsToggle(){
        try{
          let tgl = U("payrollSavePdfsToggle");
          if(tgl) return tgl;

          const side = U("payrollMatchSideMenu");
          if(!side) return null;

          const block = document.createElement("div");
          block.className = "sideBlock";
          block.innerHTML = `
            <label class="toggleSwitch payrollSavePdfsToggle" title="Se attivo, salva i PDF su Firebase (accesso riservato)">
              <input type="checkbox" id="payrollSavePdfsToggle" checked />
              <span>Vuoi salvare i PDF?</span>
            </label>
          `;

          const afterBlock = (U("payrollShowIssues")?.closest?.(".sideBlock"))
            || (U("payrollSaveEmailsToggle")?.closest?.(".sideBlock"));

          if(afterBlock && afterBlock.parentNode === side){
            afterBlock.insertAdjacentElement("afterend", block);
          }else{
            const bottom = side.querySelector(".sideBottom");
            if(bottom) side.insertBefore(block, bottom);
            else side.appendChild(block);
          }

          tgl = block.querySelector("#payrollSavePdfsToggle");
          return tgl || U("payrollSavePdfsToggle");
        }catch(_e){
          return null;
        }
      }

      try{ ensurePayrollSavePdfsToggle(); }catch(_e){}


      // Switch (menu laterale): salva i PDF su Firebase (default ON)
      const payrollSavePdfsToggle = U("payrollSavePdfsToggle");
      if(payrollSavePdfsToggle && !payrollSavePdfsToggle.__bound){
        payrollSavePdfsToggle.__bound = true;

        try{
          N.payroll = N.payroll || {};
          N.payroll.admin = N.payroll.admin || {};

          // Default ON (se non c'è preferenza salvata)
          const stored = payrollReadSavePdfsPref();
          N.payroll.admin.savePdfs = stored;
          payrollSavePdfsToggle.checked = !!stored;
        }catch(_e){
          payrollSavePdfsToggle.checked = true;
          try{ N.payroll = N.payroll || {}; N.payroll.admin = N.payroll.admin || {}; N.payroll.admin.savePdfs = true; }catch(_e2){}
        }

        payrollSavePdfsToggle.addEventListener("change", ()=>{
          const on = !!payrollSavePdfsToggle.checked;
          try{
            N.payroll = N.payroll || {};
            N.payroll.admin = N.payroll.admin || {};
            N.payroll.admin.savePdfs = on;
            payrollWriteSavePdfsPref(on);
          }catch(_e){}

          // Best-effort: se ON, prepara Storage (ma non bloccare la UI)
          if(on){
            try{
              if(!payrollUserCanPersistPdfs()){
                showToast("Salvataggio PDF", "Per salvare i PDF devi essere autenticato con email.", 4200);
              }else{
                ensureFirebaseStorageReady().then(()=>{}).catch(()=>{});
              }
            }catch(_e){}
          }
        });
...
const btnToggleSent = U("btnPayrollToggleSent");
      if(btnToggleSent && !btnToggleSent.__bound){
        ...
        btnToggleSent.addEventListener("click", ()=>{
          // ✅ Se utente non Premium (prova gratuita già usata): invece del popup, vai alla pagina Premium
          try{
            const u = N.user;
            const isAuthed = !!(u && !u.isAnonymous);
            const isPremium = !!u?.isPremium;
            const freeUsed = !!u?.payrollFreeUsed;
            const currentShowSent = !!(N.payroll?.admin?.filters?.showSent);
            const willShowSent = !currentShowSent;
            if(isAuthed && !isPremium && freeUsed && willShowSent){
              location.href = "/premium.html";
              return;
            }
          }catch(_e){}
      ...
      // ===== UI: salvataggio email (MATCH) — spinner + mini didascalia ("mail salvata") =====
      const __PAYROLL_SAVEEMAILS_SYNC__ = { pending:0, timer:null };

      function __ensurePayrollSaveEmailsSyncStyle(){
        try{
          if(document.getElementById("ilp-payroll-saveemails-sync-style")) return;
          const style = document.createElement("style");
          style.id = "ilp-payroll-saveemails-sync-style";
          style.textContent = `
            .ilp-payroll-saveemails-sync{display:inline-flex;align-items:center;gap:6px;margin-left:10px;font-size:12px;font-weight:850;color:rgba(15,23,42,.62);white-space:nowrap}
            .ilp-payroll-saveemails-sync .ico{width:16px;height:16px;display:inline-flex;align-items:center;justify-content:center}
            .ilp-payroll-saveemails-sync .dot{width:10px;height:10px;border-radius:999px;border:2px solid currentColor;opacity:.25;box-sizing:border-box}
            .ilp-payroll-saveemails-sync .spin{width:10px;height:10px;border-radius:999px;border:2px solid currentColor;border-top-color:transparent;animation:ilp-payroll-mini-spin .75s linear infinite;box-sizing:border-box}
            .ilp-payroll-saveemails-sync .ok,.ilp-payroll-saveemails-sync .err{font-size:12px;line-height:1}
            .ilp-payroll-saveemails-sync .txt{display:none}
            .ilp-payroll-saveemails-sync[data-state="saving"] .txt,
            .ilp-payroll-saveemails-sync[data-state="ok"] .txt,
            .ilp-payroll-saveemails-sync[data-state="err"] .txt{display:inline}
            @keyframes ilp-payroll-mini-spin{to{transform:rotate(360deg)}}
          `;
          (document.head || document.documentElement).appendChild(style);
        }catch(_e){}
      }

      function ensurePayrollSaveEmailsSyncIndicator(){
        try{
          let el = document.getElementById("payrollSaveEmailsSyncIndicator");
          if(el) return el;

          const toggle = document.getElementById("payrollSaveEmailsToggle");
          if(!toggle) return null;

          __ensurePayrollSaveEmailsSyncStyle();

          el = document.createElement("span");
          el.id = "payrollSaveEmailsSyncIndicator";
          el.className = "ilp-payroll-saveemails-sync";
          el.setAttribute("role","status");
          el.setAttribute("aria-live","polite");
          el.dataset.state = "idle";
          el.innerHTML = '<span class="ico"><span class="dot"></span></span><span class="txt"></span>';

          const label = toggle.closest("label");
          if(label){
            const spans = label.querySelectorAll("span");
            const textSpan = spans && spans.length ? spans[spans.length - 1] : null;
            if(textSpan) textSpan.insertAdjacentElement("afterend", el);
      ...
      function setPayrollSaveEmailsSyncState(state, text){
        const el = ensurePayrollSaveEmailsSyncIndicator();
        if(!el) return;
        const s = String(state || "idle");
        el.dataset.state = s;
        ...
        if(ico){
          if(s === "saving") ico.innerHTML = '<span class="spin"></span>';
          else if(s === "ok") ico.innerHTML = '<span class="ok">✓</span>';
          else if(s === "err") ico.innerHTML = '<span class="err">⚠</span>';
          else ico.innerHTML = '<span class="dot"></span>';
        }

        // Tooltip coerente (no riferimenti tecnici)
        try{
          if(s === "saving") el.title = msg || "salvataggio…";
          else if(s === "ok") el.title = msg || "mail salvata";
          else if(s === "err") el.title = msg || "non salvata";
          else el.title = "pronto";
        }catch(_e){}
      }

      function beginPayrollSaveEmailsSync(text){
        try{
          __PAYROLL_SAVEEMAILS_SYNC__.pending = (__PAYROLL_SAVEEMAILS_SYNC__.pending || 0) + 1;
          if(__PAYROLL_SAVEEMAILS_SYNC__.timer){
            clearTimeout(__PAYROLL_SAVEEMAILS_SYNC__.timer);
            __PAYROLL_SAVEEMAILS_SYNC__.timer = null;
          }
          setPayrollSaveEmailsSyncState("saving", text || "salvataggio…");
        }catch(_e){}
      }

      function endPayrollSaveEmailsSync(ok){
        try{
          __PAYROLL_SAVEEMAILS_SYNC__.pending = Math.max(0, (__PAYROLL_SAVEEMAILS_SYNC__.pending || 0) - 1);

          if((__PAYROLL_SAVEEMAILS_SYNC__.pending || 0) > 0){
            setPayrollSaveEmailsSyncState("saving", "salvataggio…");
            return;
          }

          setPayrollSaveEmailsSyncState(ok ? "ok" : "err", ok ? "mail salvata" : "non salvata");

          __PAYROLL_SAVEEMAILS_SYNC__.timer = setTimeout(()=>{
            try{
              if((__PAYROLL_SAVEEMAILS_SYNC__.pending || 0) === 0){
                setPayrollSaveEmailsSyncState("idle", "");
              }
            }catch(_e){}
          }, ok ? 2200 : 3200);
        }catch(_e){}
      ...
      async function persistRowToDirectory(row, opts = {}) {
        ...
        // UI: mostra spinner in sidebar mentre salviamo/verifichiamo su Firebase
        try{ beginPayrollSaveEmailsSync(); }catch(_e){}
        ...
        try{
          ...
          await api.setDoc(api.doc(db, COL_PAYROLL_DIRECTORY, payrollDirDocId(em)), payload, { merge:true });

          // Cleanup vecchio record (best-effort, solo se confident)
          if(oldDocId && oldDocId !== em){
            try{
              const oldRef = api.doc(db, COL_PAYROLL_DIRECTORY, payrollDirDocId(oldDocId));
          ...
          // Verifica "andata e ritorno": leggi il doc appena scritto per confermare salvataggio
          let verified = true;
          try{
            const vRef = api.doc(db, COL_PAYROLL_DIRECTORY, payrollDirDocId(em));
            const vSnap = await api.getDoc(vRef);
            if(!(vSnap && vSnap.exists())){
              verified = false;
            }else{
              const d = vSnap.data() || {};
              const em2 = normalizeEmail(d?.emailLower || d?.id || "");
              const uid2 = String(d?.ownerUid || "");
              if(em2 !== em) verified = false;
              if(uid2 && uid2 !== N.user.uid) verified = false;
            }
          }catch(_e){
            verified = false;
          }
          ...
          // UI: esito salvataggio/verifica su Firebase (mini spinner accanto a "Vuoi salvare mail")
          try{ endPayrollSaveEmailsSync(!!verified); }catch(_e){}
        }catch(err){
          console.warn("persistRowToDirectory", err);
          try{ endPayrollSaveEmailsSync(false); }catch(_e){}
          // Se Firestore fallisce, l’email NON viene salvata (solo memoria in questa sessione)
          try{
            const low = String(err?.message || "").toLowerCase();
            const body = (err?.code === "permission-denied" || low.includes("permission"))
              ? "Permessi insufficienti: impossibile salvare l’email."
              : "Impossibile salvare l’email. Riprova.";
            try{ showToast("Salvataggio email", body, 4200); }catch(_e){}
          }catch(_e){}
        }
      }
...
function __pdfPreviewClamp(n, min, max){
  n = Number(n);
  if(!isFinite(n)) n = min;
  return Math.min(max, Math.max(min, n));
}
function __pdfPreviewRound2(n){
  n = Number(n);
  if(!isFinite(n)) return 0;
  return Math.round(n * 100) / 100;
}
function __pdfPreviewGetState(){
  N.ui = N.ui || {};
  N.ui.pdfPreview = N.ui.pdfPreview || {};
  const st = N.ui.pdfPreview;

  if(!(typeof st.zoom === "number" && isFinite(st.zoom))) st.zoom = 1;
  if(!(typeof st.minZoom === "number" && isFinite(st.minZoom))) st.minZoom = 0.5;
  if(!(typeof st.maxZoom === "number" && isFinite(st.maxZoom))) st.maxZoom = 3;

  return st;
}
function __pdfPreviewUpdateUi(){
  const st = __pdfPreviewGetState();
  const total = Math.max(0, Math.round(Number(st.totalPages || 0)));
  let cur = Math.max(1, Math.round(Number(st.currentPage || 1)));
  if(total > 0) cur = Math.min(cur, total);

  const ind = U("pdfPreviewPageIndicator");
  if(ind){
    ind.textContent = total > 0 ? `Pagina ${cur} / ${total}` : "Pagina — / —";
  }

  const prevBtn = U("btnPdfPrevPage");
  const nextBtn = U("btnPdfNextPage");
  if(prevBtn) prevBtn.disabled = !(total > 0 && cur > 1);
  if(nextBtn) nextBtn.disabled = !(total > 0 && cur < total);

  const z = Number(st.zoom || 1);
  const zOut = U("btnPdfZoomOut");
  const zIn = U("btnPdfZoomIn");
  if(zOut) zOut.disabled = z <= (st.minZoom || 0.5) + 1e-6;
  if(zIn) zIn.disabled = z >= (st.maxZoom || 3) - 1e-6;
}
function __pdfPreviewBindScroll(){
  const wrap = U("pdfPreviewCanvasWrap");
  if(!wrap || wrap.dataset.pvBound === "1") return;
  wrap.dataset.pvBound = "1";

  wrap.addEventListener("scroll", ()=>{
    const st = __pdfPreviewGetState();
    if(st._scrollRaf) return;
    st._scrollRaf = requestAnimationFrame(()=>{
      st._scrollRaf = 0;
      try{ __pdfPreviewSyncCurrentPageFromScroll(); }catch(_e){}
    });
  }, { passive:true });
}
function __pdfPreviewSyncCurrentPageFromScroll(){
  ...
  const st = __pdfPreviewGetState();
  const papers = Array.isArray(st._paperEls) ? st._paperEls : [];
  if(!papers.length) return;

  const targetY = wrap.scrollTop + (wrap.clientHeight * 0.25);
  let cur = 1;
  for(let i=0;i<papers.length;i++){
    const el = papers[i];
    if(!el) continue;
    const top = (el.offsetTop || 0);
    if(top <= targetY) cur = i+1;
    else break;
  }

  if(cur !== st.currentPage){
    st.currentPage = cur;
    __pdfPreviewUpdateUi();
  }
}
function __pdfPreviewScrollToPage(pageNum, behavior="smooth"){
  const overlay = U("pdfPreviewOverlay");
  const wrap = U("pdfPreviewCanvasWrap");
  if(!overlay || overlay.getAttribute("aria-hidden") !== "false") return;
  if(!wrap) return;
  ...
  const p = Math.min(total, Math.max(1, Math.round(Number(pageNum || 1))));
  const el = wrap.querySelector(`.pdfPreviewPaper[data-page="${p}"]`);
  if(el){
    try{
      el.scrollIntoView({ behavior: behavior || "smooth", block:"start" });
    }catch(_e){
      try{ wrap.scrollTop = el.offsetTop || 0; }catch(_e2){}
    }
  }
...
function __pdfPreviewCancelRenders(){
  const st = __pdfPreviewGetState();
  try{
    const tasks = Array.isArray(st._renderTasks) ? st._renderTasks : [];
    for(const t of tasks){
      try{ t?.cancel?.(); }catch(_e){}
    }
  }catch(_e){}
  st._renderTasks = [];
}
function __pdfPreviewSetZoom(nextZoom, { keepPage } = {}){
  const st = __pdfPreviewGetState();
  const minZ = (typeof st.minZoom === "number" && isFinite(st.minZoom)) ? st.minZoom : 0.5;
  const maxZ = (typeof st.maxZoom === "number" && isFinite(st.maxZoom)) ? st.maxZoom : 3;

  const z = __pdfPreviewRound2(__pdfPreviewClamp(nextZoom, minZ, maxZ));
  if(Math.abs((Number(st.zoom || 1)) - z) < 0.001) return;

  st.zoom = z;
  __pdfPreviewUpdateUi();

  const url = String(st.directUrl || st.objectUrl || "").trim();
  if(!url) return;

  const kp = keepPage || st.currentPage || 1;
  renderPdfPreview(url, { keepPage: kp, reason: "zoom" });
}

async function renderPdfPreview(url, opts = {}){
  const overlay = U("pdfPreviewOverlay");
  const wrap = U("pdfPreviewCanvasWrap");
  const frame = U("pdfPreviewFrame");
  const controls = U("pdfPreviewControls");
  ...
  // prepara stato + scroll binding
  const st = __pdfPreviewGetState();
  __pdfPreviewBindScroll();

  // interrompi render in corso (senza distruggere il PDF se è lo stesso URL)
  __pdfPreviewCancelRenders();

  // token per interrompere render se chiudi/cambi PDF
  const token = String(Date.now()) + ":" + Math.random().toString(16).slice(2);
  st._renderToken = token;

  const keepPage = Math.max(1, Math.round(Number(opts?.keepPage || st.currentPage || 1)));
  ...
  try{ if(controls){ controls.style.display = "flex"; } }catch(_e){}

  st.totalPages = 0;
  st.currentPage = keepPage;
  st._paperEls = [];
  __pdfPreviewUpdateUi();
  ...
  // PDF.js
  let pdfjs = null;
  try{ pdfjs = await __payrollGetPDFJS(); }catch(_e){ pdfjs = null; }
  ...
  if(!pdfjs){
    ...
    try{ if(controls) controls.style.display = "none"; }catch(_e){}
    return;
  }

  // Se stiamo renderizzando lo stesso PDF, riusa doc/task (utile per zoom)
  const sameSrc = (String(st._pdfUrl || "") === src) && st._pdfTask && st._pdfDoc;
  if(!sameSrc){
    // interrompi eventuale task precedente
    try{
      if(st._pdfTask && typeof st._pdfTask.destroy === "function"){
        st._pdfTask.destroy();
      }
    }catch(_e){}
    st._pdfTask = null;
  ...
  try{
    ...
    if(st._renderToken !== token) return;
    if(overlay.getAttribute("aria-hidden") !== "false") return;

    st.totalPages = Math.max(0, Math.round(Number(pdf.numPages || 0)));
    st.currentPage = Math.min(Math.max(1, keepPage), st.totalPages || 1);
    __pdfPreviewUpdateUi();

    const dprRaw = Math.max(1, window.devicePixelRatio || 1);
    const dpr = Math.min(2, dprRaw); // evita canvas giganteschi su device DPR molto alto

    // calcola width disponibile (tolto padding)
    let padL = 0, padR = 0;
    try{
    ...
    const available = Math.max(320, (wrap.clientWidth || window.innerWidth || 1024) - padL - padR);
    const targetCssWidth = Math.min(920, available);

    const zoom = Number(st.zoom || 1) || 1;

    const papers = [];
    const renderTasks = [];
    st._renderTasks = renderTasks;

    for(let pageNum = 1; pageNum <= (st.totalPages || 0); pageNum++){
      if(st._renderToken !== token) break;
      if(overlay.getAttribute("aria-hidden") !== "false") break;

      const page = await pdf.getPage(pageNum);

      const baseVp = page.getViewport({ scale: 1 });
      const fitScale = targetCssWidth / (baseVp.width || 1);
      ...
      paper.className = "pdfPreviewPaper";
      paper.setAttribute("data-page", String(pageNum));
      paper.appendChild(canvas);

      const pageWrap = document.createElement("div");
      pageWrap.className = "pdfPreviewPage";
      pageWrap.appendChild(paper);
    ...
    // sync scroll / indicator sul page corrente
    try{
      __pdfPreviewScrollToPage(st.currentPage || 1, opts?.behavior || "auto");
    }catch(_e){}
...
function showPdfPreview({ title, sub, srcUrl, fileName, markOpenedVia } = {}){
        const overlay = U("pdfPreviewOverlay");
        const frame = U("pdfPreviewFrame");
        const nameEl = U("pdfPreviewFileName");
        if(!overlay || !frame) return;

        // Guard: su pagine che non caricano il CSS del viewer, evita che il markup resti nel flusso sotto al footer
        try{ overlay.style.display = "block"; }catch(_e){}
        ...
        st.directUrl = nextUrl;
        st.objectUrl = (nextUrl && String(nextUrl).startsWith("blob:")) ? nextUrl : "";
        st.fileName = fileName || "busta_paga.pdf";
        st.title = title || "Anteprima PDF";
        st.sub = sub || "—";

        // UI: mostra nome file in top bar
        try{
          if(nameEl) nameEl.textContent = st.fileName || st.title || "documento.pdf";
        }catch(_e){}
        ...
        try{ U("pdfPreviewControls") && (U("pdfPreviewControls").style.display = "flex"); }catch(_e){}

        overlay.setAttribute("aria-hidden","false");
        document.body.classList.add("modalOpen");
        updateModalOpenState();

        // Render del PDF (multi pagina + zoom)
        setTimeout(()=>{ try{ renderPdfPreview(st.directUrl || "", { keepPage: 1, behavior:"auto" }); }catch(_e){} }, 0);
...
function closePdfPreview(){
        ...
        try{ overlay.style.display = "none"; }catch(_e){}
        try{ overlay.setAttribute("inert",""); }catch(_e){}
        document.body.classList.remove("modalOpen");
        updateModalOpenState();

        // stop render tasks / worker
        try{ __pdfPreviewCancelRenders(); }catch(_e){}
        ...
        try{ __pdfPreviewUpdateUi(); }catch(_e){}

        setTimeout(()=>{ try{ window.__lastFocusPdf__?.focus?.(); }catch(_e){} }, 10);
      }


/* PDF preview — guard globale: su pagine senza CSS evita che il markup del viewer resti visibile sotto al footer */
(function(){
  const apply = ()=>{
    try{
      ...
      // Rimuovi controlli non attivi / legacy (se presenti in vecchie pagine o cache)
      try{ document.getElementById("btnPdfPreviewOpenWith")?.remove?.(); }catch(_e){}
      try{ document.getElementById("pdfPreviewOpenWithBtn")?.remove?.(); }catch(_e){}
      try{ document.getElementById("btnPdfPreviewMore")?.remove?.(); }catch(_e){}
      try{
        document.querySelectorAll('#pdfPreviewOverlay .pdfOpenWithBtn, #pdfPreviewOverlay [data-pdf-fake="1"]').forEach(el=>{
          try{ el.remove(); }catch(_e2){}
        });
      }catch(_e){}

      // Stato display coerente anche senza CSS
      const isOpen = ov.getAttribute("aria-hidden") === "false";
      try{ ov.style.display = isOpen ? "block" : "none"; }catch(_e){}

      // CSS guard: override di eventuali patch che forzano display:block quando aria-hidden=true
      if(!document.getElementById("ilpPdfPreviewGuardCss_20260212")){
        const st = document.createElement("style");
        st.id = "ilpPdfPreviewGuardCss_20260212";
        st.textContent = `
          #pdfPreviewOverlay[aria-hidden="true"]{ display:none !important; }
          #pdfPreviewOverlay[aria-hidden="false"]{ display:block !important; }
        `;
        try{ document.head && document.head.appendChild(st); }catch(_e){}
      }
    }catch(_e){}
  };
...
function buildSafePdfName(base){
      ...
      // ===== Premium entitlements watch (source of truth = Firestore/entitlements) =====
      function stopPayrollEntitlementsWatch(){
        try{
          const w = N.payroll?.entitlementsWatch;
          if(w && typeof w.unsub === "function"){
            try{ w.unsub(); }catch(_e){}
          }
        }catch(_e){}
        try{ if(N.payroll) N.payroll.entitlementsWatch = null; }catch(_e){}
      }

      function startPayrollEntitlementsWatch(){
        stopPayrollEntitlementsWatch();
        if(!(N.firebase?.ok && N.user && !N.user.isAnonymous)) return;
        ...
        N.payroll = N.payroll || {};
        N.payroll.entitlementsWatch = { unsub:null };

        try{
          const ref = api.doc(db, COL_PAYROLL_USAGE, uid);
          N.payroll.entitlementsWatch.unsub = api.onSnapshot(ref, snap=>{
            try{
              if(!N.user) return;

              if(!snap || !snap.exists()){
                // Doc non presente: trial disponibile (non blocchiamo la UI)
                N.user.payrollUsageLoaded = true;
                N.user.payrollUsageLoading = false;
                N.user.payrollUsageError = "";
              }else{
          ...
          // Nota: eventuale migrazione legacy (emailLower → uid) gestita altrove; qui ascoltiamo SOLO uid.
            ...
            // Se Firestore non è leggibile, fallback backend (/entitlements)
            try{
              if(N.user){
                N.user.payrollUsageError = (err?.code || err?.message || "entitlements_watch_failed");
                N.user.payrollUsageLoaded = true;
                N.user.payrollUsageLoading = false;
              }
            }catch(_e){}
            try{ updateAuthUI(); }catch(_e){}
            try{ setTimeout(()=>{ try{ tryLoadPayrollEntitlementsFromBillingBackend(); }catch(_e){} }, 250); }catch(_e){}
          });
        }catch(_e){}
      }
      ...
      function onSignedIn(){
        ...
        // Entitlements (Premium / prova gratuita): source of truth = Firestore (/payrollUsage) o GET /entitlements
        try{ readLocalPayrollEntitlements(); }catch(_e){}
        try{ updateAuthUI(); }catch(_e){}
        try{ startPayrollEntitlementsWatch(); }catch(_e){}
        try{ loadPayrollEntitlementsFromServer(); }catch(_e){}
        try{
          const u = new URL(location.href);
          const p = u.searchParams;

          // Rientro da Stripe Checkout: sincronizza Premium in modo robusto
          const premiumFlag = String(p.get("premium") || "").toLowerCase();
          const sid = p.get("session_id") || p.get("sid") || p.get("checkout_session_id") || p.get("cs") || "";
          const succ = String(p.get("success") || "").toLowerCase();
          const isSuccess = (premiumFlag === "success") || (succ === "1" || succ === "true" || succ === "ok" || succ === "yes") || !!sid;
          const isCancel = (premiumFlag === "cancel") || (p.get("premium_cancel") === "1") || (String(p.get("cancel")||"") === "1") || (String(p.get("canceled")||"") === "1");
          ...
          if(isCancel){
            ...
            // ✅ Se Stripe ti riporta su una pagina "strana" (es. /dashboard.html?premium=cancel),
            //    porta l’utente su "Diventa Premium" ed evita loop/lag.
            try{
              const isPremiumPage = /\/premium\.html$/i.test(u.pathname);
              if(!isPremiumPage){
                const base = new URL("./", import.meta.url);
                const t = new URL("premium.html", base);
                t.searchParams.set("premium","cancel");
                location.replace(t.toString());
                return;
              }
            }catch(_e){}
          }else{
            const shouldSync = isSuccess || fromStripe || !!lsPending || startedRecent;
            const startedAt = (() => {
              try{
                // Segnale "forte" di rientro dal checkout: parte da adesso (evita timeout prematuri)
                if(isSuccess || fromStripe || startedRecent || !!sid) return Date.now();
                const ts = Number(lsPending?.ts || 0);
            ...
            if(shouldSync){
              ...
              try{
                if(N.user){
                  ...
                  N.user.premiumSyncStartedAt = startedAt;
                  N.user.payrollUsageLoaded = false;
                  N.user.payrollUsageLoading = false;
                  N.user.payrollUsageError = "";
                }
              ...
              try{ updateAuthUI(); }catch(_e){}
              try{
                const msg = isSuccess ? "Pagamento completato. Sto sincronizzando l’abbonamento…" : "Verifico lo stato dell’abbonamento…";
                showToast("Premium", msg, 2600);
              }catch(_e){}
              try{ if(shouldRefreshMarker) ilpMarkPremiumCheckoutPending(sid || (lsPending?.session_id || "")); }catch(_e){}
              try{ loadPayrollEntitlementsFromServer(true); }catch(_e){}
            }else{
              // Safety net: forziamo una verifica token una tantum (utile se il backend non passa query params)
              try{
                const k = "ilp_premium_forcecheck_ts_v1";
          ...
          // Pulisci URL (evita ripetizioni al refresh)
          try{
            const had = (p.has("premium") || p.has("success") || p.has("session_id") || p.has("sid") || p.has("checkout_session_id") || p.has("cs") || p.has("premium_cancel") || p.has("cancel") || p.has("canceled"));
            if(had){
              ["premium","success","session_id","sid","checkout_session_id","cs","premium_cancel","cancel","canceled"].forEach(k=>{ try{ p.delete(k); }catch(_e){} });
      ...
      function onSignedOut(){
        try{ stopPayrollEntitlementsWatch(); }catch(_e){}
    ...
    async function initFirebase(){
      try{
        ...
        for(const a of cdnAttempts){
          try{
            // PERF: carica solo i moduli necessari al primo paint (Storage/AppCheck sono lazy)
            const wantsStorage = (()=>{ 
              try{
                const v = localStorage.getItem("payroll_save_pdfs_v1");
                // default ON se non impostato
                if(v === null || v === undefined) return true;
                return String(v) !== "0";
              }catch(_e){
                // fallback legacy
                try{ return !!(typeof PAYROLL_PERSIST_UPLOADS !== "undefined" && PAYROLL_PERSIST_UPLOADS); }catch(_e2){ return false; }
              }
            })();
            mods = await Promise.all([
              ...
              wantsStorage ? import(a.base + "firebase-storage.js") : Promise.resolve(null)
            ]);
            loadedBase = a.base;
            loadedFrom = a.name;
            break;
        ...
        const [{ initializeApp, getApps, getApp }, authMod, fsMod, storageMod] = mods;
        let appCheckMod = null;
        try{ console.info("✅ Firebase SDK loaded from", loadedFrom); }catch(_e){}
        const app = (getApps().length ? getApp() : initializeApp(firebaseConfig));

        // --- Firebase App Check (lazy) ---
        let appCheck = null;
        try{ if(globalThis.__PAGHEIA_APPCHECK__) appCheck = globalThis.__PAGHEIA_APPCHECK__; }catch(_e){}
        let __appCheckInitWarned = false;
        let __appCheckInitPromise = null;

        async function ensureAppCheck(forceRefresh=false){
          try{
            // già inizializzato (altre pagine/route)
            if(globalThis.__PAGHEIA_APPCHECK__){
              appCheck = globalThis.__PAGHEIA_APPCHECK__;
              try{ if(state && state.firebase) state.firebase.appCheck = appCheck; }catch(_e){}
            }
            // Se abbiamo già istanza + API (getToken), non serve altro
            try{
              if(appCheck && state && state.firebase && state.firebase.appCheckApi && typeof state.firebase.appCheckApi.getToken === "function"){
                return appCheck;
              }
            }catch(_e){}
            if(appCheck && appCheckMod && typeof appCheckMod.getToken === "function"){
              try{ if(state && state.firebase) state.firebase.appCheckApi = { getToken: appCheckMod.getToken, getLimitedUseToken: appCheckMod.getLimitedUseToken }; }catch(_e){}
              return appCheck;
            }

            if(!APPCHECK_ENABLED) return null;
            if(!APPCHECK_SITE_KEY || String(APPCHECK_SITE_KEY).includes("RECAPTCHA_SITE_KEY_HERE")) return null;

            if(__appCheckInitPromise) return await __appCheckInitPromise;

            __appCheckInitPromise = (async ()=>{
              // Carica modulo (preferisci il CDN già funzionante)
              const bases = [];
              try{ if(loadedBase) bases.push(loadedBase); }catch(_e){}
              try{
                for(const c of (cdnAttempts || [])){
                  if(c && c.base && c.base !== loadedBase) bases.push(c.base);
                }
              }catch(_e){}
              ...
              if(!appCheckMod) throw lastErr || new Error("firebase-app-check.js non disponibile");
              ...
              // Stato runtime (per header X-Firebase-AppCheck)
              try{
                if(state && state.firebase){
                  state.firebase.appCheck = appCheck;
                  state.firebase.appCheckApi = { getToken: appCheckMod.getToken, getLimitedUseToken: appCheckMod.getLimitedUseToken };
                }
              }catch(_e){}
              return appCheck;
            })();

            return await __appCheckInitPromise;
          }catch(err){
            if(!__appCheckInitWarned){
              __appCheckInitWarned = true;
              console.warn("App Check init (lazy)", err);
            }
            return null;
          }
        }
...
const auth = authMod.getAuth(app);
        ...
        const storage = storageMod ? storageMod.getStorage(app) : null;
        state.firebase = {
          ok: true,
          app,
          auth,
        ...
        // =========================
        const OTP_BASE = String(globalThis.ILP_AUTH_OTP_BASE || "https://ilp-auth-otp-162609991629.europe-west1.run.app").trim().replace(/\/+$/,"");
        ...
        function setOtpStage(stage="request"){
          ...
          try{
            ...
            if(t) t.textContent = (emailOtpStage === "verify") ? "Verifica codice" : "Invia codice";
          }catch(_e){}
          try{
            const wrap = document.getElementById("authOtpCodeWrap");
            if(wrap) wrap.style.display = (emailOtpStage === "verify") ? "" : "none";
          }catch(_e){}
          try{
...
if(emailOtpStage !== "verify"){
        ...
        // Compatibilità: in giro il codice chiama ancora setEmailMode("send"/"complete")
        function setEmailMode(mode="send", _unused=""){
          const m = String(mode||"send").toLowerCase();
          setOtpStage((m === "verify" || m === "complete") ? "verify" : "request");
        }
        ...
        function getEmailInputValue(){
          try{ return String(document.getElementById("authEmailInput")?.value || "").trim(); }
          catch(_e){ return ""; }
        }
        function getCodeInputValue(){
          try{ return String(document.getElementById("authCodeInput")?.value || "").trim(); }
          catch(_e){ return ""; }
        }
        function isValidEmail(v){
          const s = String(v||"").trim();
        ...
        function isValidOtpCode(v){
          const s = String(v||"").trim();
          return /^\d{6}$/.test(s);
        }

        async function postOtp(path, body){
          const url = OTP_BASE + String(path||"");
        ...
        const ac = state.firebase?.appCheck;
            ...
            console.warn("appcheck token (otp)", err);
          }
          ...
          if(!r.ok || !data || data.ok !== true){
            const msg = (data && (data.error || data.message)) || (r.status === 429 ? "Troppe richieste. Attendi e riprova." : "Operazione non riuscita.");
        ...
        async function requestOtpCode(email, { silentToast=false } = {}){
          ...
          try{
            try{ markAuthInflight("Invio codice", "email_otp"); }catch(_e){}
            await postOtp("/auth/request-code", { email: mail });
            ...
            if(!silentToast){
              try{ showToast("Codice inviato", "Ti abbiamo inviato un codice a 6 cifre via email. Inseriscilo per completare l’accesso.", 4200); }catch(_e){}
            }
          }catch(err){
            console.warn("request-code", err);
            try{
        ...
        async function verifyOtpAndLogin(email, code){
          const mail = String(email||"").trim();
          ...
          if(!isValidOtpCode(c)){
            setEmailError("Inserisci un codice valido (6 cifre).");
            return;
          }
          ...
          try{
            try{ markAuthInflight("Accesso", "email_otp"); }catch(_e){}
            const data = await postOtp("/auth/verify-code", { email: mail, code: c });

            const customToken = String(data.customToken || "");
            if(!customToken) throw new Error("Token non disponibile.");
            ...
            // redirect dove stavi (best effort)
            try{
              let next = sanitizeSameOriginUrl(getAuthNextUrl()) || "";
            ...
            try{ showToast("Accesso completato", "Sei entrato correttamente.", 2400); }catch(_e){}
          }catch(err){
            console.warn("verify-code", err);
            try{
        ...
        async function handleEmailAuthAction(){
          ...
          if(emailOtpStage === "verify"){
            const code = getCodeInputValue();
            await verifyOtpAndLogin(email || emailOtpLastEmail, code);
          }else{
            try{ setAuthNextUrl(location.href); }catch(_e){}
...
// Esponi azioni auth (usate anche dal CTA upload)
        ...
        try{
          ...
          // OTP UI
          document.getElementById("authCodeInput")?.addEventListener("keydown", (ev)=>{
            try{
              if(ev && ev.key === "Enter"){
                ev.preventDefault();
...
try{
          ...
          document.getElementById("btnOtpChangeEmail")?.addEventListener("click", ()=>{
            try{
              setOtpStage("request");
              setEmailError("");
              setEmailSent(false);
          ...
          // Logout
          document.getElementById("btnLogoutBottom")?.addEventListener("click", (ev)=>{ try{ ev.preventDefault(); ev.stopPropagation(); }catch(_e){} signOutGoogle(); });
          document.getElementById("btnLogout")?.addEventListener("click", (ev)=>{ try{ ev.preventDefault(); ev.stopPropagation(); }catch(_e){} signOutGoogle(); });
document.getElementById("btnSecurityClose")?.addEventListener("click", ()=>toggleSecurityOverlay(false));
        ...
        // Email OTP: nessun link da gestire
...
// Auth gate: se la sessione esiste entri, altrimenti chiedi login Google.
        ...
        authMod.onAuthStateChanged(auth, async user=>{
          ...
          if(user){
            ...
            // PERF: AppCheck solo quando serve (evita reCAPTCHA al primo paint)
            try{ state.firebase?.ensureAppCheck?.().catch(()=>{}); }catch(_e){}
    ...
    function isHomeFile(file){
      ...
      return (!f || f === "index.html" || f === "index.html");
    }
    ...
    function getSpaRouteFromLocation(){
      try{
        // Fallback robusto: supporta anche URL del tipo "/?open=dashboard" (utile per ritorno da Stripe)
        try{
          const u = new URL(location.href);
          const open = String(u.searchParams.get("open") || u.searchParams.get("route") || "").toLowerCase().trim();
          if(open === "dashboard") return "dashboard";
        ...
        if(!relative || relative === "index.html" || relative === "index.html") return "home";
    ...
    function getSpaUrlForRoute(route){
      ...
      // ⚠️ Nota: usiamo "?open=dashboard" per rendere il link robusto anche su hosting senza rewrite di /dashboard.
      if(route === "dashboard") return `${base}?open=dashboard`;
      return base;
    }
    ...
    async function ensureDashboardModule(){
      // Dashboard è embedded nel bundle: se esiste già, inizializza e basta.
      if(typeof globalThis.initPagheiaDashboard === "function"){
        try{ globalThis.initPagheiaDashboard(); }catch(_e){}
        return;
      }
      if(!spaState.dashboardModulePromise){
        spaState.dashboardModulePromise = import(new URL("./dashboard.js?v=20260130_008", import.meta.url).toString())
...
async function renderSpaRoute(route){
      ...
      // Spinner: transitions (dashboard load, module init, ecc.)
      try{ globalThis.ilpNavSpinner?.showDelayed?.(140); }catch(_e){}
      ...
      try{ document.title = "iLovePaghe"; }catch(_e){}
        ...
        try{ globalThis.ilpNavSpinner?.hide?.(); }catch(_e){}
      }
    }
    ...
    function initSpaRouter(){
      try{
        ...
        globalThis.ilovepagheSpaNavigate = navigateSpa;
        ...
        document.addEventListener("click", (ev)=>{
          try{
            ...
            if(!a) return;
// Se questa pagina NON contiene il contenitore SPA (es. pagine marketing standalone),
// non intercettare i link data-spa: fai navigazione "hard" così il menu utente va a Dashboard.
const appViewEl = document.getElementById("appView");
if(!appViewEl){
  const hrefHard = a.getAttribute("href");
  if(hrefHard != null){
    ev.preventDefault();
    try{ location.href = new URL(hrefHard, location.href).href; }catch(_e){}
  }
  return;
}
    ...
    // =========================
    // Hash router — #home, #terms, #privacy-policy (no reload, stesso header/footer)
    // =========================
    let __hashViewLast = "";
    let __hashViewApplying = false;

    // Modal-route: #carica (o #upload) apre la modale "Carica" e permette il BACK per tornare alla view precedente.
    let __caricaOpenedFromUi = false;

    function isCaricaHash(h){
      const v = String(h||"").trim().replace(/^#/, "");
      return v === "carica" || v === "upload";
    }
// In #carica NON vogliamo più mostrare lo step "idle" (Carica e analizza).
// La view corretta è la workflow/lista (match) con menu laterale.
function ensureCaricaWorkflowStep(){
  try{
    const a = (typeof N !== "undefined" && N && N.payroll && N.payroll.admin) ? N.payroll.admin : null;
    if(!a) return;
    const cur = String(a.step || "").trim() || "idle";

    // Se sta già lavorando, non forzare altro
    if(cur === "extracting") return;

    // Se siamo già nelle viste "giuste", non toccare
    if(cur === "match" || cur === "sending" || cur === "history" || cur === "directory") return;

    // Altrimenti: salta l'idle/preview e porta alla lista (workflow)
    const desired = (a.lastFlowStep && (a.lastFlowStep === "sending" || a.lastFlowStep === "match"))
      ? a.lastFlowStep
      : "match";

    try{ if(typeof Di === "function") Di(desired); }catch(_e){}
    try{ if(desired === "match" && typeof Pi === "function") Pi(); }catch(_e){}
  }catch(_e){}
}

function openCaricaModalWorkflow(){
  try{ toggleAdminModal(true, "adminArea"); }catch(_e){}
  try{ ensureCaricaWorkflowStep(); }catch(_e){}
}

    function openCaricaRoute(){
      // Legacy: la route "#carica" non esiste più. Non tocchiamo l'URL.
      try{
        const u = new URL(location.href);
        const raw = String(u.hash || "").trim().replace(/^#/, "");
        if(raw === "carica" || raw === "upload"){
          u.hash = "";
          history.replaceState({ ...(history.state||{}), spa:true, route:"home" }, "", u.href);
        }
      }catch(_e){}
      try{ openCaricaModalWorkflow?.(); }catch(_e){}
    }

    // Chiude la modale e "esce" dalla route #carica.
    // - Se la route è stata aperta da UI (e quindi esiste una entry precedente), usa history.back()
    // - Se è un deep link, rimuove l'hash e resta sulla home.
    function closeCaricaRouteIfNeeded(preferBack=true){
      // Legacy: non esiste più alcuna route #carica — chiudi e pulisci hash se presente
      try{ toggleAdminModal(false); }catch(_e){}
      try{
        const u = new URL(location.href);
        const raw = String(u.hash || "").trim().replace(/^#/, "");
    ...
    function getHashView(){
      try{
        ...
        if(h === "carica" || h === "upload") return "home";
        return "home";
      }catch(_e){
        return "home";
      }
...
function applyHashView(){
      ...
      try{
        const incomingHash = String(location.hash || "").trim().replace(/^#/, "");
        if(incomingHash === "come-funziona"){
          try{ location.replace("/come-funziona.html"); }catch(_e){}
          return;
        }
        // Legacy cleanup: la vecchia route #carica/#upload NON esiste più
        try{
          const raw = String(location.hash || "").trim().replace(/^#/, "");
          if(raw === "carica" || raw === "upload"){
            const u = new URL(location.href);
            u.hash = "";
            history.replaceState({ ...(history.state||{}), spa:true, route:"home" }, "", u.href);
        ...
        if(view !== "home" && (!home || !terms || !privacy)){
          try{
        ...
        // Scroll top solo quando cambia la view contenuto
        if(__hashViewLast !== contentView){
          __hashViewLast = contentView;
          try{
            window.scrollTo({ top:0, left:0, behavior:"auto" });
          }catch(_e){
            try{ window.scrollTo(0,0); }catch(_e2){}
...
/* =========================
   Dipendenti (pagina standalone) — payrollDirectory
   - Colonne: Nome, Codice fiscale, Buste paga inviate, Mail
   - Unico campo editabile: Mail (salvataggio immediato su Firestore, niente locale)
   ========================= */
/* =========================
   Dipendenti (pagina standalone)
   ========================= */
(function initIlovepagheEmployeesPage(){
  ...
  const elTbody  = $("empTbody");
  const elMeta   = $("empMeta");
  const elNotice = $("empNotice");
  const elSearch = $("empSearch");
  ...
  const COL_PAYROLL_DIRECTORY   = "payrollDirectory";
  const COL_PAYROLL_DOCS        = "payrollDocs";
  const COL_PAYROLL_DOCS_LEGACY = "payrolls";
  const CAP_DOCS = 8000;

  let unsubDir = null;

  // key (preferibilmente fiscalCode) -> rec
  let current = new Map();

  // key -> { tr, input, actions, btnSave, btnCancel, ... }
  let rows = new Map();

  // stats
  let statsByCF    = Object.create(null); // cf -> { count, lastMs }
  let statsByEmail = Object.create(null); // email -> { count, lastMs }
  let statsLoading = false;
  let lastStatsAt  = 0;

  const saveLocks = new Set(); // key

  function setNotice(text, tone=""){
    if(!elNotice) return;
    const msg = String(text || "").trim();
    elNotice.textContent = msg;
    elNotice.style.display = msg ? "" : "none";
    elNotice.classList.remove("ok","warn","bad");
    if(tone) elNotice.classList.add(tone);
  }

  function setMeta(text){
    if(elMeta) elMeta.textContent = String(text || "");
  }

  function setRowStatus(tr, state, msg){
    if(!tr) return;
    tr.dataset.saveState = String(state || "");
    const cell = tr.querySelector?.("[data-emp-status]");
    if(cell) cell.textContent = String(msg || "");
  }

  function stableKeyFrom(rec){
    // Priorità: codice fiscale (stabile anche se cambia email)
    const cf = normalizeCF(rec?.fiscalCode);
    if(cf) return cf;
    // fallback (meno ideale): docId
    return String(rec?.docId || rec?.id || rec?.emailLower || "").trim();
  }

  function renderEmpty(msg){
    if(!elTbody) return;
    elTbody.innerHTML = `<tr class="empEmptyRow"><td colspan="5">${esc(msg || "Nessun dipendente.")}</td></tr>`;
    rows.clear();
  }

  function applyFilterToRow(tr, rec){
    const q = String(elSearch?.value || "").trim().toLowerCase();
    if(!q){ tr.style.display = ""; return; }
    const hay = `${rec.displayName||""} ${rec.fiscalCode||""} ${rec.emailLower||""}`.toLowerCase();
    tr.style.display = hay.includes(q) ? "" : "none";
  }

  function applyFilterAll(){
    for(const [key, rec] of current.entries()){
      const row = rows.get(key);
      if(row?.tr) applyFilterToRow(row.tr, rec);
    }
  }

  function pickStatsFor(rec){
    const cf = normalizeCF(rec?.fiscalCode);
    if(cf && statsByCF[cf]) return statsByCF[cf];
    const em = normalizeEmail(rec?.emailLower || rec?.email || "");
    if(em && statsByEmail[em]) return statsByEmail[em];
    return null;
  }

  function updateStatsCells(key, rec){
    const row = rows.get(key);
    if(!row?.tr) return;

    const cntEl  = row.tr.querySelector?.("[data-emp-count]");
    const lastEl = row.tr.querySelector?.("[data-emp-last]");

    const s = pickStatsFor(rec);
    if(cntEl)  cntEl.textContent  = s ? String(s.count ?? "—") : "—";
    if(lastEl) lastEl.textContent = s ? formatMs(s.lastMs || 0) : "—";
  }

  function markDirty(row, isDirty){
    row.isDirty = !!isDirty;
    if(row.actions){
      row.actions.hidden = !row.isDirty;
    }
  }

  function setRowInteractive(row, enabled){
    const on = !!enabled;
    try{ if(row.input) row.input.disabled = !on; }catch(_e){}
    try{ if(row.btnSave) row.btnSave.disabled = !on; }catch(_e){}
    try{ if(row.btnCancel) row.btnCancel.disabled = !on; }catch(_e){}
  }

  async function saveEmailChange(key){
    const row = rows.get(key);
    const rec = current.get(key);
    if(!row || !rec) return;

    if(saveLocks.has(key)) return;
    saveLocks.add(key);
    ...
    try{
      ...
      const prev = normalizeEmail(row.originalEmail || "");
      const em = normalizeEmail(row.draftEmail || input?.value || "");
      if(!isValidEmail(em)) throw new Error("Email non valida.");
      if(em === prev){
        markDirty(row, false);
        setRowStatus(tr, "", "");
        return;
      ...
      if(!api || !db) throw new Error("Firebase non pronto.");

      const oldDocId = String(rec.docId || rec.id || "").trim();
      const newDocId = uid + "__" + em;

      // Conflitto: email già usata da un altro dipendente (stesso owner)
      if(newDocId !== oldDocId){
        try{
          const otherSnap = await api.getDoc(api.doc(db, COL_PAYROLL_DIRECTORY, newDocId));
          if(otherSnap?.exists?.()){
            const other = otherSnap.data() || {};
            const otherCF = normalizeCF(other.fiscalCode);
            const thisCF = normalizeCF(rec.fiscalCode);
            if(otherCF && thisCF && otherCF !== thisCF){
              throw new Error("Questa email è già usata da un altro dipendente.");
            }
          }
        }catch(err){
          // se non possiamo leggere, non blocchiamo in modo aggressivo
          const code = String(err?.code || "").toLowerCase();
          if(code !== "permission-denied") throw err;
        }
      }

      setRowStatus(tr, "saving", "Salvo…");
      setRowInteractive(row, false);
      ...
      const payload = {
        ...
        updatedAtClient: Date.now(),
        updatedBy: String(state?.user?.email || "").toLowerCase()
      };

      // Mantieni identità (read-only su questa pagina)
      payload.displayName = String(rec.displayName || rec.fullName || "—").trim() || "—";
      payload.fiscalCode  = String(rec.fiscalCode  || "").trim().toUpperCase();

      // Best-effort: mantieni campi extra se esistono
      if(rec.firstName != null) payload.firstName = String(rec.firstName || "");
      if(rec.lastName  != null) payload.lastName  = String(rec.lastName  || "");
      if(rec.fullNameNorm != null) payload.fullNameNorm = String(rec.fullNameNorm || "");

      if(typeof api.serverTimestamp === "function") payload.updatedAt = api.serverTimestamp();

      // 1) crea/aggiorna il doc nuovo (docId = uid__emailLower)
      await api.setDoc(api.doc(db, COL_PAYROLL_DIRECTORY, newDocId), payload, { merge:true });

      // 2) se docId cambia: cancella il vecchio (regola attuale)
      if(oldDocId && newDocId !== oldDocId){
        try{ await api.deleteDoc(api.doc(db, COL_PAYROLL_DIRECTORY, oldDocId)); }catch(_e){}
      }

      // Aggiorna stato locale (senza aspettare snapshot)
      rec.docId = newDocId;
      rec.emailLower = em;

      row.originalEmail = em;
      row.draftEmail = em;
      markDirty(row, false);

      try{ if(input) input.value = em; }catch(_e){}
      setRowStatus(tr, "ok", "Salvata");
      setTimeout(()=>{ try{ setRowStatus(tr, "", ""); }catch(_e){} }, 1400);
    }catch(err){
      console.warn("employees save email", err);
      const msg = String(err?.message || "Errore").slice(0, 90);
      setRowStatus(row.tr, "err", msg);
      // resta in edit mode: non resettiamo il valore
    }finally{
  ...
  function upsertRow(key, rec){
    ...
    if(!row){
      ...
      tr.dataset.empKey = key;

      tr.innerHTML = `
        <td class="nameCell" data-emp-name></td>
        <td class="cfCell" data-emp-cf></td>
        <td class="sentCell" data-emp-count>—</td>
        <td class="lastCell" data-emp-last>—</td>
        <td class="emailCell">
          <div class="emailWrap">
            <input class="emailInput" type="email" inputmode="email" autocomplete="email" data-emp-mail />
            <div class="mailActions" data-emp-actions hidden>
              <button type="button" class="miniBtn" data-emp-save>Salva</button>
              <button type="button" class="miniBtn" data-emp-cancel>Annulla</button>
            </div>
            <span class="mailState" data-emp-status aria-live="polite"></span>
          </div>
        </td>
      `;
      ...
      const input   = tr.querySelector?.("[data-emp-mail]");
      const actions = tr.querySelector?.("[data-emp-actions]");
      const btnSave = tr.querySelector?.("[data-emp-save]");
      const btnCancel = tr.querySelector?.("[data-emp-cancel]");
      ...
      if(input){
        input.addEventListener("focus", ()=>{
          ...
          try{
            ...
            if(!row.isDirty) row.originalEmail = v;
            row.draftEmail = normalizeEmail(input.value || "");
          }catch(_e){}
        });
        ...
        input.addEventListener("input", ()=>{
          try{
            row.draftEmail = normalizeEmail(input.value || "");
            input.classList.toggle("invalid", !!(input.value || "").trim() && !isValidEmail(row.draftEmail));
            const dirtyNow = (row.draftEmail !== normalizeEmail(row.originalEmail || ""));
            markDirty(row, dirtyNow);
            setRowStatus(tr, "", "");
          }catch(_e){}
        });
      }
      ...
      if(btnCancel){
        btnCancel.addEventListener("click", ()=>{
          try{
            const prev = normalizeEmail(row.originalEmail || "");
            if(row.input) row.input.value = prev;
            if(row.input) row.input.classList.toggle("invalid", !!prev && !isValidEmail(prev));
            row.draftEmail = prev;
            markDirty(row, false);
            setRowStatus(tr, "", "");
          }catch(_e){}
      ...
      if(btnSave){
        btnSave.addEventListener("click", async ()=>{
          try{
            const em = normalizeEmail(row.draftEmail || row.input?.value || "");
            if(!isValidEmail(em)){
              setRowStatus(tr, "err", "Email non valida");
              if(row.input) row.input.classList.add("invalid");
              return;
            }
            const prev = normalizeEmail(row.originalEmail || "");
            if(em === prev){
            ...
            const ok = window.confirm("Confermi la modifica dell'email?");
            if(!ok) return;
            await saveEmailChange(key);
          }catch(_e){}
        });
      }
    }

    // Aggiorna celle testo
    const nameEl = row.tr.querySelector?.("[data-emp-name]");
    const cfEl   = row.tr.querySelector?.("[data-emp-cf]");

    if(nameEl) nameEl.textContent = rec.displayName || "—";
    if(cfEl)   cfEl.textContent   = rec.fiscalCode  || "—";

    // Email: aggiorna SOLO se non stiamo editando/dirty
    const emailLower = normalizeEmail(rec.emailLower || rec.email || "");
    if(row.input){
      if(!row.isDirty && document.activeElement !== row.input){
        row.input.value = emailLower || "";
        row.originalEmail = emailLower || "";
        row.draftEmail = emailLower || "";
        row.input.classList.toggle("invalid", !!emailLower && !isValidEmail(emailLower));
      }
    }

    updateStatsCells(key, rec);
    applyFilterToRow(row.tr, rec);
  }

  function removeRow(key){
    const row = rows.get(key);
    if(row?.tr) row.tr.remove();
    rows.delete(key);
  }
  ...
  async function loadStats(force=false){
    ...
    const now = Date.now();
    if(!force && lastStatsAt && (now - lastStatsAt) < 45000) return; // throttle
    ...
    try{
      ...
      const cols = [COL_PAYROLL_DOCS, COL_PAYROLL_DOCS_LEGACY];

      const seenCF = Object.create(null);    // cf -> Set(unique)
      const seenEm = Object.create(null);    // email -> Set(unique)
      const lastCF = Object.create(null);    // cf -> ms
      const lastEm = Object.create(null);    // email -> ms

      for(const col of cols){
        try{
          // IMPORTANT: filtra per ownerUid, altrimenti la query può fallire con permission-denied
          const q = api.query(api.collection(db, col), api.where("ownerUid","==", uid), api.limit(CAP_DOCS));
          const snap = await api.getDocs(q);

          snap?.forEach(docSnap=>{
            const d = docSnap.data() || {};

            const cf = normalizeCF(d.fiscalCode || "");
            const em = normalizeEmail(d.emailLower || "");

            const mk = String(d.monthKey || "").trim();
            const uniq = mk || docSnap.id;

            if(cf){
              if(!seenCF[cf]) seenCF[cf] = new Set();
              seenCF[cf].add(uniq);
            }
            if(em){
              if(!seenEm[em]) seenEm[em] = new Set();
              seenEm[em].add(uniq);
            }

            const ms =
              (typeof d.sentAtClient === "number" && d.sentAtClient) ? d.sentAtClient :
              (typeof d.updatedAtClient === "number" && d.updatedAtClient) ? d.updatedAtClient :
              (typeof d.createdAtClient === "number" && d.createdAtClient) ? d.createdAtClient :
              tsToMs(d.sentAt) || tsToMs(d.updatedAt) || tsToMs(d.createdAt) || tsToMs(d.sentAtISO) || 0;

            if(ms){
              if(cf) lastCF[cf] = Math.max(lastCF[cf] || 0, ms);
              if(em) lastEm[em] = Math.max(lastEm[em] || 0, ms);
            }
          });
        }catch(err){
          const code = String(err?.code || "").toLowerCase();
          if(code !== "permission-denied") console.warn("employees stats load", col, err);
        }
      }

      const outCF = Object.create(null);
      for(const cf of Object.keys(seenCF)){
        outCF[cf] = { count: seenCF[cf].size, lastMs: lastCF[cf] || 0 };
      }

      const outEm = Object.create(null);
      for(const em of Object.keys(seenEm)){
        outEm[em] = { count: seenEm[em].size, lastMs: lastEm[em] || 0 };
      }
      ...
      console.warn("employees stats", err);
    }finally{
      statsLoading = false;
    }
  }
  ...
  function stopWatch(){
    try{ if(typeof unsubDir === "function") unsubDir(); }catch(_e){}
    unsubDir = null;
    current.clear();
    rows.clear();
    statsByCF = Object.create(null);
    statsByEmail = Object.create(null);
    lastStatsAt = 0;
  }
  ...
  function startWatch(){
    ...
    const uid = String(state?.user?.uid || "").trim();
    if(!uid || !(state?.firebase?.ok)) return;

    const api = state.firebase.api;
    const db = state.firebase.db;
    if(!api || !db) return;

    renderEmpty("Caricamento…");
    setMeta("Caricamento…");
    setNotice("", "");

    const q = api.query(api.collection(db, COL_PAYROLL_DIRECTORY), api.where("ownerUid","==", uid));
    unsubDir = api.onSnapshot(q, (snap)=>{
      const seenKeys = new Set();
      const next = new Map();

      try{ if(elTbody && elTbody.querySelector(".empEmptyRow")) elTbody.innerHTML = ""; }catch(_e){}

      snap?.forEach(docSnap=>{
        const d = docSnap.data() || {};
        const docId = docSnap.id;

        let emailLower = normalizeEmail(d.emailLower || d.email || "");
        if(!emailLower){
          try{
            const parts = String(docId || "").split("__");
            if(parts.length >= 2) emailLower = normalizeEmail(parts.slice(1).join("__"));
          }catch(_e){}
        }
        ...
        const rec = {
          ...
          id: docId,
          emailLower,
          displayName: String(d.displayName || d.fullName || "—").trim() || "—",
          fiscalCode: String(d.fiscalCode || "").trim().toUpperCase() || "—",
          enabled: (d.enabled !== false),
          firstName: d.firstName,
          lastName: d.lastName,
          fullNameNorm: d.fullNameNorm
        };

        const key = stableKeyFrom(rec);
        next.set(key, rec);
        seenKeys.add(key);
        upsertRow(key, rec);
      });
      ...
      if(current.size === 0){
        renderEmpty("Nessun dipendente.");
      }

      setMeta(`Dipendenti: ${current.size}`);
      applyFilterAll();

      // Stats (buste inviate + ultimo invio)
      loadStats();
    }, (err)=>{
      console.warn("employees directory watch", err);
      renderEmpty("Accesso non disponibile.");
      setMeta("");
      setNotice("Accesso non disponibile ai dipendenti (permessi o rete).", "warn");
    });
  }

  async function boot(){
    try{
      const waitFirebaseReady = async (timeoutMs = 12000)=>{
        const t0 = Date.now();
        while(true){
          try{ if(state?.firebase?.ok) return true; }catch(_e){}
          if((Date.now() - t0) > timeoutMs) return false;
          await new Promise(r=> setTimeout(r, 80));
        }
      };

      const fbOk = await waitFirebaseReady();
      if(!fbOk){
        renderEmpty("Firebase non pronto.");
        setMeta("");
        setNotice("Firebase non pronto: ricarica la pagina.", "warn");
        return;
      }

      try{ elSearch?.addEventListener("input", ()=>{ applyFilterAll(); }); }catch(_e){}

      // refresh leggero (niente UI extra): mantiene ultimo invio abbastanza fresco
      try{
        setInterval(()=>{
          if(document.visibilityState !== "visible") return;
          // evita refresh se sto salvando qualcosa
          if(saveLocks.size) return;
          loadStats(true);
        }, 60000);
      }catch(_e){}
      ...
      const handle = ()=>{
        ...
        stopWatch();
        renderEmpty("Accesso richiesto.");
        setMeta("");
        setNotice("Accedi per gestire i dipendenti.", "warn");
      };

      if(authApi && auth && typeof authApi.onAuthStateChanged === "function"){
        authApi.onAuthStateChanged(auth, ()=>{ try{ handle(); }catch(_e){} });
      }else{
        let n = 0;
        const t = setInterval(()=>{
          n++;
          if(state?.authHydrated || n > 80){
            clearInterval(t);
            try{ handle(); }catch(_e){}
          }
        }, 100);
      ...
      console.warn("employees boot", err);
      renderEmpty("Errore inizializzazione.");
      setMeta("");
      setNotice("Errore inizializzazione.", "bad");
    }
  }
