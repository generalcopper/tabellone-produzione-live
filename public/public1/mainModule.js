...
/****************************************************************
    ...
    let state = {
      ...
      productUoms: {}, // per-code unità di misura (offline fallback) — pz|kg|ton
        ...
        try {
          if (fb.user && fb.db) {
            ...
            for (const it of prodMap.values()) {
              ...
              // migra productUoms (fallback locale)
              try{
                const oLow = String(oldCode || "").trim().toLowerCase();
                if (oLow && state && state.productUoms && (oLow in state.productUoms) && !(newLow in state.productUoms)) {
                  state.productUoms[newLow] = state.productUoms[oLow];
                  delete state.productUoms[oLow];
                  saveLocalData();
                }
              }catch(_){}
...
function __getLastSupplierInfoForCode(code){
  ...
  return {
    ...
    async function deleteProductByCode(code, opts) {
      ...
      try {
        if (state && state.productUoms && (low in state.productUoms)) {
          delete state.productUoms[low];
          saveLocalData();
        }
      } catch(_) {}
...
function normalizeUom(v){
      const s = String(v || "").trim().toLowerCase();
      return (s === "pz" || s === "kg" || s === "ton") ? s : "";
    }
    function uomLabel(v){
      const s = normalizeUom(v);
      if (!s) return "";
      return s.toUpperCase();
    }
    function getUomForCode(code){
      const low = String(code || "").trim().toLowerCase();
      const p = findProductByCode(code);
      const fromProducts = normalizeUom(p && (p.uom || p.unit || p.um));
      const fromLocal = normalizeUom(state.productUoms && state.productUoms[low]);
      return fromProducts || fromLocal || "";
    }
    async function setProductUomForCode(code, uom, nameHint){
      const key = String(code || "").trim();
      const low = key.toLowerCase();
      if (!key) return;
      ...
      try{
        ...
        const nm = String(nameHint || "").trim();
        if (nm) { patch.name = nm; patch.nameLower = nm.toLowerCase(); }

        await setDoc(doc(fb.db, "orgs", ORG_ID, "products", keyToDocId(low)), patch, { merge: true });
        renderAll(); renderAnag();
      }catch(e){
        console.error("setProductUomForCode failed", e);
        showToast("Errore salvataggio unità di misura", "err");
      }
    }

    async function setProductUomForCodes(codes, uom, nameHint){
      const list = (Array.isArray(codes) ? codes : []).map(x => String(x || "").trim()).filter(Boolean);
      if (!list.length) return;

      const u = normalizeUom(uom);

      // Offline fallback + optimistic local (una sola volta)
      state.productUoms = state.productUoms || {};
      ...
      try{
        ...
        await Promise.all(list.map(async (key) => {
          ...
          // nameHint solo sul primo (evita overwrite)
          if (nm && key === list[0]) { patch.name = nm; patch.nameLower = nm.toLowerCase(); }
          await setDoc(doc(fb.db, "orgs", ORG_ID, "products", keyToDocId(low)), patch, { merge: true });
        }));
      }catch(e){
...
let __stockRowByKey = new Map();
    ...
    function renderStockTable(stockArr) {
      ...
      stockTbody.innerHTML = show.map(r => {
        ...
        const qtyVal = safeInt(r.qty);        const isGroup = (r.__isAliasGroup && Array.isArray(r.__codes) && r.__codes.length > 1);

        // U.M. (unità di misura)
        let uom = "";
        try{
          if (isGroup && Array.isArray(r.__codes) && r.__codes.length) {
            const set = new Set();
            for (const c of r.__codes) {
              const u = normalizeUom(getUomForCode(c));
              if (u) set.add(u);
            }
            if (set.size === 1) uom = Array.from(set)[0];
          } else {
            uom = normalizeUom(getUomForCode(r.code));
          }
        }catch(_){ uom = ""; }
        const uomPill = uom
          ? `<span class="uomTag" title="Unità di misura">${escapeHtml(uomLabel(uom) || uom)}</span>`
          : `<span class="uomTag uomEmpty" title="Unità di misura non definita">—</span>`;
...
try{
        ...
        if (anagTbody) anagTbody.innerHTML = `<tr><td class="td-muted" colspan="5">Modulo prodotti non caricato.</td></tr>`;
        return;
      }
...
async function deleteSupplierCascade(supplierId){
  ...
  try{
    ...
    function openProductModal(id, ctx) {
      ...
      // U.M. (unità di misura): per singolo codice o gruppo alias (se coerente)
      let __uom = "";
      try{
        const scope = (isAliasGroup && ctx && Array.isArray(ctx.__codes) && ctx.__codes.length) ? ctx.__codes : [code];
        const set = new Set();
        for (const c of scope) {
          const u = normalizeUom(getUomForCode(c));
          if (u) set.add(u);
        }
        if (set.size === 1) __uom = Array.from(set)[0];
      }catch(_){ __uom = normalizeUom(getUomForCode(code)); }
  ...
  `);
      ...
      baseFields.push(`
        <div class="field" style="grid-column: 1 / -1;">
          ...
          <div class="td-muted" style="margin-top:6px;">
            ${isAliasGroup ? "Si applica a tutti i codici del gruppo alias." : "Classifica come pezzi, kg o tonnellate."}
          </div>
        </div>
      `);
...
// Danger zone: elimina prodotto (solo da Anagrafica Prodotti)
      ...
      if (uSel) {
        ...
        uSel.addEventListener("change", async () => {
          ...
          const nm = (document.getElementById("prodNameEdit")?.value || title);
          if (isAliasGroup) await setProductUomForCodes((ctx && ctx.__codes) || [], v, nm);
          else await setProductUomForCode(code, v, nm);
          showToast("Unità di misura salvata");
        });
      }
