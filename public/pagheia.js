...
globalThis.ILOVEPAGHE_BUNDLE_BUILD = "20260201_020_otp_auth";

// dashboard.js — iLovePaghe (SPA dashboard)
// Build: 20260131_013_dirfix
globalThis.ILOVEPAGHE_DASHBOARD_BUILD = "20260201_020_otp_auth";
const initPagheiaDashboard = () => {
  ...
  async function initFirebase(){
    ...
    let appCheck = null;
    try{
      // Evita doppia init (pagheia.js può averlo già inizializzato)
...
const auth = getAuth(app);
  ...
  try{
    ...
    return location.origin + "/?premium=success";
  }
})();
const cancelUrl = (() => {
  try{
...
// Build: 20260130_008
    globalThis.ILOVEPAGHE_BUILD = "20260201_012_uploadfast";
...
const firebaseConfig = {
    ...
    const APPCHECK_ENABLED = (() => {
        ...
        })();const PAYROLL_EXTRACT_ENDPOINT = "https://gemini-pdf-extract-rgjpp5trna-ew.a.run.app/extract";
...
const LS_PAYROLL_FREE_USED = "ilovepaghe_payroll_free_used_v1";
      ...
      function ji() {
        ...
        // ✅ Se c’è un’estrazione/upload in corso, interrompi (evita update tardivi e "sensazione di lentezza")
        try{ if(e.__extractXhr){ e.__extractXhr.abort(); } }catch(_e){}
        try{ e.__extractXhr = null; }catch(_e){}
        try{ e.__extractToken = ""; }catch(_e){}
...
function __payrollLoadScript(src){
      ...
      function refreshGroupedValidation(){
        ...
        if(matchResultsTitle){
          ...
          const companyCodeText = companyCodeLabel ? ` (cod. ${Ao(companyCodeLabel)})` : "";

          const companyHtml = (uniqCompaniesNorm.length===1 && companyLabel)
            ? `<span class="rmSep">·</span><span class="rmCompany">azienda ${Ao(companyLabel)}${companyCodeText}</span>`
            : (uniqCompaniesNorm.length>1 ? `<span class="rmSep">·</span><span class="rmCompany">aziende diverse</span>` : (companyCodeLabel ? `<span class="rmSep">·</span><span class="rmCompany">cod. ${Ao(companyCodeLabel)}</span>` : ""));

          if(uniqMonths.length === 1){
            const mLabel = formatPayrollMonthLabel(uniqMonths[0]);
            matchResultsTitle.innerHTML = `<span class="rmMain">Da inviare: ${Ao(countLabel)},</span><span class="rmMonth">mese ${Ao(mLabel)}</span>${companyHtml}`;
          }else if(uniqMonths.length > 1){
            matchResultsTitle.innerHTML = `<span class="rmMain">Da inviare: ${Ao(countLabel)}</span><span class="rmSep">·</span><span class="rmMonth">(mesi diversi)</span>${companyHtml}`;
          }else{
            matchResultsTitle.innerHTML = `<span class="rmMain">Da inviare: ${Ao(countLabel)}</span>${companyHtml}`;
          }
        }
...
function renderUploadHistory(){
      ...
      async function Vi() {
        ...
        // Token anti-race: se fai Reset o carichi un altro PDF, ignoriamo gli esiti tardivi
        const __token = String(Date.now()) + "_" + Math.random().toString(16).slice(2);
        try{ e.__extractToken = __token; }catch(_e){}

        // Se c’era un XHR in corso, abort (evita doppie elaborazioni e UI "lenta")
        try{ if(e.__extractXhr) e.__extractXhr.abort(); }catch(_e){}
        try{ e.__extractXhr = null; }catch(_e){}
        ...
        try {
          ...
          if (!isPdf) return Ve("Formato non valido", "Carica un file PDF."), void(t0 && setProgress(0,"Errore formato"));
          ...
          try{ e.originalPdfBytesPromise = (async ()=>{
            const buf = await file.arrayBuffer();
            // Non sovrascrivere se nel frattempo è partito un nuovo upload
            try{ if(e.__extractToken === __token) e.originalPdfBytes = buf; }catch(_e){}
            return buf;
          })(); }catch(_e){ try{ e.originalPdfBytesPromise = null; }catch(_e2){} }
          ...
          try{
            e.sourceFileHashPromise = (async ()=>{
              try{
                ...
                try{ if(e.__extractToken === __token) e.sourceFileHash = hex; }catch(_e){}
                return hex;
              }catch(_e){ return ""; }
            })();
          }catch(_e){}
          ...
          // ✅ Upload immediato: invia il File direttamente (niente arrayBuffer -> Blob prima dell'upload)
          ...
          const uploadWithProgress = (fd)=> new Promise((resolve,reject)=>{
            try{
              ...
              try{ e.__extractXhr = xhr; }catch(_e){}
              ...
              xhr.upload.onprogress = (evt)=>{
                try{
                  if(evt && evt.lengthComputable){
                    ...
                    setProgress(Math.max(8, Math.min(60, p)), "Caricamento…");
                  }else{
                    setProgress(25, "Caricamento…");
                  }
                }catch(_e){}
              };
              ...
              xhr.upload.onload = ()=>{
                uploadDone = true;
                try{ setProgress(62, "Analisi in corso…"); }catch(_e){}
              };

              // Durante la risposta (server processing) teniamo vivo il feedback
              xhr.onprogress = ()=>{
                if(uploadDone){
                  try{ setProgress(80, "Analisi in corso…"); }catch(_e){}
                }
              };

              xhr.onload = ()=>{
                try{ if(e.__extractToken !== __token) return resolve(""); }catch(_e){}
              ...
              xhr.onabort = ()=>reject(new Error("Operazione annullata"));

              xhr.send(fd);
            }catch(err){ reject(err); }
          });

          setProgress(8, "Caricamento…");
          const raw = await uploadWithProgress(formData);

          // Se nel frattempo è partito un nuovo upload/reset, non toccare lo stato
          try{ if(e.__extractToken !== __token) return; }catch(_e){}
          ...
          dbg && (dbg.textContent = `Estrazione OK · ${Math.round(performance.now() - t0)}ms`);
          ...
          const toZeroBasedPages = (arr)=>{ 
            ...
            const assumeOneBased = !hasZero && min>=1;
          ...
          t && (t.textContent = `PREVIEW (${pagesFlat.length})`);
          dbg && (dbg.textContent = `Estrazione OK · pagine ${pagesFlat.length} · ${Math.round(performance.now() - t0)}ms`);
          setProgress(100, "Analisi completata");
          ...
          // Se nel frattempo è partito un nuovo upload/reset, ignora questo errore
          try{ if(e.__extractToken !== __token) return; }catch(_e){}
          ...
          // Non spegnere "loading" se nel frattempo è partita un’altra estrazione
          try{
            if(e.__extractToken === __token){
              e.gemini.loading = !1;
              e.__extractXhr = null;
            }
          }catch(_e){}
...
// === Modalità privacy (NO Storage): invio diretto senza archiviazione ===
      ...
      U("btnCloseAdmin")?.addEventListener("click", ()=>{ try{ toggleAdminModal(false); }catch(_e){} });
        ...
        try{
          ...
          }else if(typeof globalThis.pagheiaSpaNavigate === "function"){
            globalThis.pagheiaSpaNavigate("home");
...
function buildSafePdfName(base){
    ...
    async function initFirebase(){
      try{
        ...
        const [{ initializeApp, getApps, getApp }, authMod, fsMod, storageMod, appCheckMod] = mods;
        ...
        let appCheck = null;
        try{
          // Evita doppia init tra pagine/route SPA
          if(globalThis.__PAGHEIA_APPCHECK__){
            appCheck = globalThis.__PAGHEIA_APPCHECK__;
          }else if(APPCHECK_ENABLED && APPCHECK_SITE_KEY && !String(APPCHECK_SITE_KEY).includes("RECAPTCHA_SITE_KEY_HERE")){
            appCheck = appCheckMod.initializeAppCheck(app, {
              provider: new appCheckMod.ReCaptchaV3Provider(APPCHECK_SITE_KEY),
              isTokenAutoRefreshEnabled: true
            });
            globalThis.__PAGHEIA_APPCHECK__ = appCheck;
          }else if(!APPCHECK_ENABLED){
            try{ console.info("ℹ️ App Check disabilitato su questo dominio:", location.hostname); }catch(_e){}
          }else{
...
function applyHashView(){
      ...
      try{
        ...
        const showHow = (contentView === "come-funziona");
